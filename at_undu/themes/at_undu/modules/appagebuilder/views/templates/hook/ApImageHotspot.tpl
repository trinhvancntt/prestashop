                ...
                <div style="position: relative;">
                    ...
                    {if isset($formAtts.items) && $formAtts.items}
                        {foreach from=$formAtts.items item=item}
                            <div class="redhotspot {$formAtts.form_id}_{$item.id}" id="hotspot_{$formAtts.form_id}_{$item.id}" style="position: absolute; z-index: 103; top: {$item.temp_top}%; left: {$item.temp_left}%;"><span>{$item.id}</span></div>
                            <script>
