	...
	<div class="rb-control-field">
		...
		<div class="rb-control-input-wrapper">
			<select data-setting="{{ data.name }}">
				<option value="">{l s='None' mod='rbthemedream'}</option>
				{foreach from=$animations key=animations_group_name item=animations_group}
					<optgroup label="{$animations_group_name}">
						{foreach from=$animations_group key=animation_name item=animation_title}
