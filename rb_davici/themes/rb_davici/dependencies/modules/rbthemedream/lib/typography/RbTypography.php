...
class RbTypography
{
	...
	public function __construct()
    {
    	...
    	$this->font_title = array(
    		self::TYPOGRAPHY_1 => $this->module->l('Primary'),
			self::TYPOGRAPHY_2 => $this->module->l('Secondary' ),
			self::TYPOGRAPHY_3 => $this->module->l('Body Text'),
			self::TYPOGRAPHY_4 => $this->module->l('Accent Text'),
    	);

    	$this->font_value = array(
    ...
    public function getFontDefault()
	{
		...
		foreach ($this->font_title as $key_f => $font) {
			$data[] = array(
				'title' => $font,
				'value' => $this->font_value[$key_f],
			);
		}
	...
	public static function rbGetFonts() {
		return array(
			// System fonts.
			'Arial' => self::SYSTEM,
			'Tahoma' => self::SYSTEM,
			...
			// Google Fonts (last update: 04/08/2019).
			'ABeeZee' => self::GOOGLE,
			'Abel' => self::GOOGLE,
	...
	public static function rbGetFontType($name) {
		if (empty(self::rbGetFonts()[$name]))
			return false;

		return self::rbGetFonts()[$name];
	}

	public static function rbGetFontsGroups($groups = []) {
		return array_filter(
			self::rbGetFonts(),
	...
	public static function rbGetFields()
	{
		$modules = new Rbthemedream();
		$fields = array();

		$fields['font_size'] = array(
			'label' => $modules->l('Size'),
			'type' => 'slider',
		...
		$fields['text_transform'] = array(
			'label' => $modules->l('Transform'),
			'type' => 'select',
			'default' => '',
			'options' => [
				'' => $modules->l('Default'),
				'uppercase' => $modules->l('Uppercase'),
				'lowercase' => $modules->l('Lowercase'),
				'capitalize' => $modules->l('Capitalize'),
			],
		);

		$fields['font_style'] = array(
			'label' => $modules->l('Style'),
			'type' => 'select',
			'default' => '',
			'options' => [
				'' => $modules->l('Default'),
				'normal' => $modules->l('Normal'),
				'italic' => $modules->l('Italic'),
				'oblique' => $modules->l('Oblique'),
			],
		);

		$fields['line_height'] = array(
			'label' => $modules->l('Line-Height'),
			'type' => 'slider',
			'default' => array(
				'unit' => 'em',
			),
		...
		$fields['letter_spacing'] = array(
			'label' => $modules->l('Letter Spacing'),
			'type' => 'slider',
			'range' => array(
				'px' => array(
					'min' => -5,
