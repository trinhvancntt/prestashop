...
require_once(_PS_MODULE_DIR_.'rbthemedream/lib/typography/RbTypography.php');

class RbWidget
{
	public function __construct()
	{
		$this->available_controls = array(
			'text',
			'number',
			'textarea',
			'select',
	...
	public function getControlsData()
	{
		...
		foreach ($this->available_controls as $control) {
			$this->name = $control;
			$controls[$control] = $this->getSettings();
			$controls[$control]['default_value'] = $this->getDefaultValue(); 
		}
	...
	public function getDefaultValue()
	{
		...
		} else if ($key == 'image_dimensions') {
			return array( 'width' => '', 'height' => '');
		} else if ($key == 'url') {
			return array('is_external' => '', 'url' => '');
		} else if ($key == 'box_shadow') {
			return array(
				'horizontal' => 0,
				'vertical' => 0,
				'blur' => 10,
	...
	public function psParseArgs($args, $defaults = '')
	{
        if (is_object($args))
   	...
   	public function getSettings($setting_key = null)
   	{
   		$base_settings = array(
			'separator' => 'default',
			'label_block' => false,
		...
		$settings = $this->psParseArgs(
			$this->getDefaultSettings($this->name),
			$base_settings
		);
	...
	public function getDefaultSettings($key)
	{
		if ($key == 'textarea' ||
			$key == 'heading' ||
			$key == 'media' ||
		...
		} else if ($key == 'dimensions' || $key == 'slider') {
			if ($key == 'dimensions') {
				return array_merge(
					$this->arrayDefaultSettings(),
					array(
						'label_block' => true,
	...
	public function arrayDefaultSettings()
	{
		return array(
			'size_units' => array('px'),
			'range' => array(
