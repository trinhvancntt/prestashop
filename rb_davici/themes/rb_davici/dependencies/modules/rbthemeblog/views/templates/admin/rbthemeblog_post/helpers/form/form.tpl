	...
	{if $input.name == "tags"}
		...
		{/if}
			...
			{/if}
					...
					value="{$rbblogpost->getTags($language.id_lang)}" />
			{if $languages|count > 1}
				</div>
				<div class="col-lg-2">
		...
		{if isset($input.display_image) && $input.display_image}
			{if isset($fields_value.featured) && $fields_value.featured}
				<div id="image">
					...
					<p align="center">{l s='File size' mod='rbthemeblog'} {$fields_value.featured_size}kb</p>
					<a class="btn btn-default" href="{$current}&{$identifier}={$form_id}&token={$token}&deleteFeatured=1">
						<i class="icon-trash"></i> {l s='Delete' mod='rbthemeblog'}
					</a>
				</div>
			{/if}
		{/if}
	...
	{elseif $input.type == 'file' && $input.name == 'cover'}
		{if isset($input.display_image) && $input.display_image}
			{if isset($fields_value.cover) && $fields_value.cover}
				<div id="image">
					{$fields_value.cover}
					<p align="center">{l s='File size' mod='rbthemeblog'} {$fields_value.cover_size}kb</p>
