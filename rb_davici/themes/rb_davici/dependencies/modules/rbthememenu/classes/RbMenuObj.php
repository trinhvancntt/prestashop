...
class RbMenuObj extends ObjectModel
{
    public $fields;
    ...
    public function renderForm()
    {
        ...
        if ($configs) {
            foreach ($configs as $key => $config) {
                ...
                $confFields = array(
                    ...
                    'img_del_link' => $this->id && isset($config['type']) && $config['type']=='file' && $this->$key!='' && @file_exists(dirname(__FILE__).'/../views/img/upload/'.$this->$key) ? $helper->module->baseAdminUrl().'&deleteimage='.$key.'&itemId='.(isset($this->id)?$this->id:'0').'&rb_object=Rb_'.Tools::ucfirst($fields_form['form']['name']) : false,
                );
        ...
        $link = new Link();
        $helper->currentIndex = $link->getAdminLink('AdminModules', true).'&configure=rbthememenu';
        ...
        if ($configs) {
            foreach ($configs as $key => $config) {
                if ($config['type']=='checkbox') {
                    $fields[$key] = $this->id ? explode(',', $this->$key) : (isset($config['default']) &&
                        $config['default'] ? $config['default'] : array());
                } elseif (isset($config['lang']) && $config['lang']) {
                    foreach ($languages as $l) {
                        $temp = $this->$key;
                        $fields[$key][$l['id_lang']] = $this->id ? $temp[$l['id_lang']] :
                        (isset($config['default']) && $config['default'] ? $config['default'] : null);
                    }
                } elseif (!isset($config['tree'])) {
                    $fields[$key] = $this->id ? $this->$key :
                    (isset($config['default']) && $config['default'] ? $config['default'] : null);
                }
            }
        }

        if (Tools::ucfirst($fields_form['form']['name']) == 'Menu') {
            $rb_object = 'RbMegaMenu';
        } else {
            $rb_object = 'RbMenu' . Tools::ucfirst($fields_form['form']['name']);
        }
        ...
        return str_replace(array('id="rbthememenu_menu_form"', 'id="fieldset_0"'), '', $helper->generateForm(array($fields_form)));
    }
    ...
    public function clearImage($image)
    {
        ...
        if (!$this->id) {
            $errors[] = Rbthememenu::$trans['object_empty'];
        } elseif (!isset($configs[$image]['type']) || isset($configs[$image]['type']) && $configs[$image]['type']!='file') {
            $errors[] = Rbthememenu::$trans['field_not_valid'];
        } elseif (isset($configs[$image]) &&
            !isset($configs[$image]['required']) ||
            (isset($configs[$image]['required']) && !$configs[$image]['required'])
        ) {
            $imageName = $this->$image;
            $imagePath = dirname(__FILE__).'/../views/img/upload/'.$imageName;

            if ($imageName && file_exists($imagePath) && !Rbthememenu::imageExits($imageName, $this->id)) {
                @unlink($imagePath);
                $this->$image = '';

                if ($this->update()) {
                    $success[] = Rbthememenu::$trans['image_deleted'];

                    if (Configuration::get('RBTHEMEMENU_CACHE_ENABLED')) {
                        Rbthememenu::clearAllCache();
                    }
                } else {
                    $errors[] = Rbthememenu::$trans['unkown_error'];
                }
            }
        } else {
            $errors[] = $configs[$image]['label']. Rbthememenu::$trans['required_text'];
        }
    ...
    public function deleteObj()
    {
        ...
        foreach ($configs as $key => $config) {
            if ($config['type'] == 'file' &&
                $this->$key &&
                @file_exists(dirname(__FILE__).'/../views/img/upload/'.$this->$key) &&
                !RBthememenu::imageExits($this->$key, $this->id)
            ) {
        ...
        if (!$this->delete()) {
            $errors[] = Rbthememenu::$trans['cannot_delete'];
        } else {
            foreach ($images as $image) {
                @unlink($image);
            }

            $success[] = Rbthememenu::$trans['item_deleted'];
            ...
            if (isset($configs['sort_order']) && $configs['sort_order']) {
                Db::getInstance()->execute(
                    "UPDATE "._DB_PREFIX_."rbthememenu_".pSQL($this->fields['form']['name'])."
            ...
            if ($this->id && isset($this->fields['form']['connect_to2']) && $this->fields['form']['connect_to2'] &&
                ($subs = Db::getInstance()->executeS("SELECT id_".pSQL($this->fields['form']['connect_to2'])." FROM "._DB_PREFIX_."rbthememenu_".pSQL($this->fields['form']['connect_to2']). " WHERE id_".pSQL($this->fields['form']['name'])."=".(int)$this->id))
            ) {
                foreach ($subs as $sub) {
                    if (Tools::ucfirst(Tools::strtolower($this->fields['form']['connect_to2'])) == 'Menu') {
                        $className = 'RbMegaMenu';
                    } else {
                        $className = 'RbMenu' . Tools::ucfirst(Tools::strtolower($this->fields['form']['connect_to2']));
                    }

                    if (class_exists($className) && $className != 'RbMenu') {
            ...
            if ($this->id && isset($this->fields['form']['connect_to']) && $this->fields['form']['connect_to'] &&
                ($subs = Db::getInstance()->executeS("SELECT id_".pSQL($this->fields['form']['connect_to'])." FROM "._DB_PREFIX_."rbthememenu_".pSQL($this->fields['form']['connect_to']). " WHERE id_".pSQL($this->fields['form']['name'])."=".(int)$this->id))
            ) {
                foreach ($subs as $sub) {
                    if (Tools::ucfirst(Tools::strtolower($this->fields['form']['connect_to2'])) == 'Menu') {
                        $className = 'RbMegaMenu';
    ...
    {
       return ($max = Db::getInstance()->getValue("SELECT max(".pSQL($key).") FROM "._DB_PREFIX_."rbthememenu_".pSQL($this->fields['form']['name']).($group && ($groupval > 0) ? " WHERE ".pSQL($group)."=".(int)$groupval : ''))) ? (int)$max : 0;
    }
    ...
    public function updateOrder($previousId = 0, $groupdId = 0, $parentObj = '')
    {
        ...
        if ($previousId > 0) {
            if (Tools::ucfirst($this->fields['form']['name']) == 'Menu' || $this->fields['form']['name'] == '') {
                $objName = 'RbMegaMenu';
            } else {
                $objName = 'RbMenu' . Tools::ucfirst($this->fields['form']['name']);
            }
        ...
        if ($this->update()) {
            Db::getInstance()->execute(
                "UPDATE "._DB_PREFIX_."rbthememenu_".pSQL($this->fields['form']['name'])."
                SET sort_order=sort_order+1 
                WHERE sort_order>=".(int)$this->sort_order."
                AND id_".pSQL($this->fields['form']['name'])."!=".(int)$this->id."
                          ".($group && $groupdId ? "
                AND ".pSQL($group)."=".(int)$this->$group : "")
            );
    ...
    public function saveData()
    {
            ...
            foreach ($configs as $key => $config) {
                ...
                if (isset($config['lang']) && $config['lang']) {
                    if (isset($config['required']) &&
                        ...
                        $errors[] = $config['label'].' '.Rbthememenu::$trans['required_text'];
                    }
                } else {
                    if (isset($config['required']) && $config['required'] && isset($config['type']) && $config['type']=='file') {
                        if ($this->$key=='' && !isset($_FILES[$key]['size'])) {
                            ...
                            if ($fileSize > 100) {
                                $errors[] = $config['label'].' '.Rbthememenu::$trans['file_too_large'];
                            }
                        }
                    } else {
                        if (isset($config['required']) && $config['required'] && $config['type']!='switch' && trim(Tools::getValue($key) == '')) {
                            $errors[] = $config['label'].' '.Rbthememenu::$trans['required_text'];
                        } elseif (!is_array(Tools::getValue($key)) &&
                            ...
                            if (!Validate::$validate(trim(Tools::getValue($key)))) {
                                $errors[] = $config['label'].' '.Rbthememenu::$trans['invalid_text'];
                            }
        ...
        if ($this->fields['form']['name']=='menu') {
            switch (Tools::getValue('link_type')) {
                case 'CUSTOM':
                    if (trim(Tools::getValue('link_'.$id_lang_default)) == '') {
                        $errors[] = Rbthememenu::$trans['custom_link_required_text'];
                    }
                    break;
                case 'CMS':
                    if (!(int)Tools::getValue('id_cms')) {
                        $errors[] = Rbthememenu::$trans['cms_required_text'];
                    }
                    break;
                case 'CATEGORY':
                    if (!(int)Tools::getValue('id_category')) {
                        $errors[] = Rbthememenu::$trans['category_required_text'];
                    }
                    break;
                case 'MNFT':
                    if (!(int)Tools::getValue('id_manufacturer')) {
                        $errors[] = Rbthememenu::$trans['manufacturer_required_text'];
                    }
                    break;
                case 'MNSP':
                    if (!(int)Tools::getValue('id_supplier')) {
                        $errors[] = Rbthememenu::$trans['supplier_required_text'];
                    }
                    break;
                case 'CONTACT':
                    break;
            ...
            if (isset($bubble_text_entered) && $bubble_text_entered) {
                if (!Tools::getValue('bubble_text_color')) {
                    $errors[] = Rbthememenu::$trans['bubble_text_color_is_required'];
                }

                if (!Tools::getValue('bubble_background_color')) {
                    $errors[] = Rbthememenu::$trans['bubble_background_color_is_required'];
                }
            }
        }
            ...
            switch (Tools::getValue('block_type')) {
                case 'HTML':
                    if (trim(Tools::getValue('content_'.$id_lang_default)) == '') {
                        $errors[] = Rbthememenu::$trans['content_required_text'];
                    }
                    break;
                case 'CMS':
                    if (!Tools::getValue('id_cmss')) {
                        $errors[] = Rbthememenu::$trans['cmss_required_text'];
                    }
                    break;
                case 'CATEGORY':
                    if (!Tools::getValue('id_categories')) {
                        $errors[] = Rbthememenu::$trans['categories_required_text'];
                    }
                    break;
                case 'MNFT':
                    if (!Tools::getValue('id_manufacturers')) {
                        $errors[] = Rbthememenu::$trans['manufacturers_required_text'];
                    }
                    break;
                case 'MNSP':
                    if (!Tools::getValue('id_suppliers')) {
                        $errors[] = Rbthememenu::$trans['suppliers_required_text'];
                    }
                    break;
                case 'PRODUCT':
                    if (Tools::getValue('product_type', false) == 'specific') {
                        if (!Tools::getValue('id_products', false)) {
                            $errors[] = Rbthememenu::$trans['products_required_text'];
                        }
                    } else {
                        if (!Tools::getValue('product_count', false)) {
                            $errors[] = Rbthememenu::$trans['product_count_required_text'];
                        } elseif (!Validate::isUnsignedId(Tools::getValue('product_count'))) {
                            $errors[] = Rbthememenu::$trans['product_count_not_valid_text'];
                        }
                    }
                    break;
                case 'IMAGE':
                    if ($this->image=='' && (!isset($_FILES['image']['size']) || isset($_FILES['image']['size']) && !$_FILES['image']['size'])) {
                        $errors[] = Rbthememenu::$trans['image_required_text'];
                    }
                    break;
                default:
                    $errors[] = Rbthememenu::$trans['block_type_not_valid_text'];
                    break;
            }
        }
            ...
            if ($configs) {
                foreach ($configs as $key => $config) {
                    if (isset($config['type']) && $config['type']=='sort_order') {
                        if (!$this->id) {
                            if (!isset($config['order_group'][$parent]) ||
                                isset($config['order_group'][$parent]) &&
                                !$config['order_group'][$parent]
                            ) {
                        ...
                        if (isset($_FILES[$key]['tmp_name']) && isset($_FILES[$key]['name']) && $_FILES[$key]['name']) {
                            ...
                            if (file_exists($fileName)) {
                                $errors[] = $config['label'].' '.Rbthememenu::$trans['file_existed'];
                            } else {
                                ...
                                if (!$errors && isset($_FILES[$key]) &&
                                    ...
                                    elseif (!$temp_name || !move_uploaded_file($_FILES[$key]['tmp_name'], $temp_name))
                                        $errors[] = Rbthememenu::$trans['can_not_upload'];
                                    elseif (!ImageManager::resize($temp_name, $fileName, null, null, $type))
                                        $errors[] = Rbthememenu::$trans['upload_error_occurred'];
                                    ...
                                    if (!$errors) {
                                        if ($this->$key!='') {
                                            ...
                                            if (file_exists($oldImage) && !Rbthememenu::imageExits($this->$key, $this->id))
                                                @unlink($oldImage);
                                        }
        ...
        if (!count($errors)) {
            if ($this->id && $this->update() || !$this->id && $this->add()) {
                if (Configuration::get('RBTHEMEMENU_CACHE_ENABLED'))
                    Rbthememenu::clearAllCache();
                $success[] = Rbthememenu::$trans['data_saved'];
            } else
                $errors[] = Rbthememenu::$trans['unkown_error'];
        }
    ...
    public function duplicateItem($id_parent = false, $id_parent2 = false)
    {
        ...
        if ($id_parent &&
            isset($this->fields['form']['parent']) &&
            ($parent = 'id_'.$this->fields['form']['parent']) &&
            property_exists($this, $parent)
        ) {
            $this->$parent = $id_parent;
        }

        if ($id_parent2 &&
            isset($this->fields['form']['parent2']) &&
            ($parent2 = 'id_'.$this->fields['form']['parent2']) &&
            property_exists($this, $parent2)
        ) {
        ...
        if (property_exists($this, 'image') &&
            $this->image &&
            file_exists(dirname(__FILE__).'/../views/img/upload/'.$this->image)
        ) {
        ...
        if (property_exists($this, 'menu_img_link') &&
            $this->menu_img_link &&
            file_exists(dirname(__FILE__).'/../views/img/upload/'.$this->menu_img_link)
        ) {
            $salt = $this->maxVal('id_'.$this->fields['form']['name'])+1;
        ...
        if(property_exists($this, 'background_image') &&
            $this->background_image &&
            file_exists(dirname(__FILE__).'/../views/img/upload/'.$this->background_image)
        ) {
            $salt = $this->maxVal('id_'.$this->fields['form']['name'])+1;
        ...
        if (property_exists($this, 'tab_img_link') &&
            $this->tab_img_link &&
            file_exists(dirname(__FILE__).'/../views/img/upload/'.$this->tab_img_link)
        ) {
            $salt = $this->maxVal('id_'.$this->fields['form']['name'])+1;
        ...
        if ($this->add()) {
            ...
            if (get_class($this)=='RbMegaMenu' && $this->enabled_vertical) {
                if (isset($this->fields['form']['connect_to2']) &&
                    $this->fields['form']['connect_to2'] &&
                    ($subs = Db::getInstance()->executeS("SELECT id_".pSQL($this->fields['form']['connect_to2'])." FROM "._DB_PREFIX_."rbthememenu_".pSQL($this->fields['form']['connect_to2']). " WHERE id_".pSQL($this->fields['form']['name'])."=".(int)$oldId))
                ) {
                    foreach ($subs as $sub) {
                        if (Tools::ucfirst(Tools::strtolower($this->fields['form']['connect_to2'])) == 'Menu') {
                            $className = 'RbMegaMenu';
                ...
                if (isset($this->fields['form']['connect_to']) &&
                    $this->fields['form']['connect_to'] &&
                    ($subs = Db::getInstance()->executeS("SELECT id_".pSQL($this->fields['form']['connect_to'])." FROM "._DB_PREFIX_."rbthememenu_".pSQL($this->fields['form']['connect_to']). " WHERE id_".pSQL($this->fields['form']['name'])."=".(int)$oldId))
                ) {
                    foreach ($subs as $sub) {
                        if (Tools::ucfirst(Tools::strtolower($this->fields['form']['connect_to'])) == 'Menu') {
                            $className = 'RbMegaMenu';
                        } else {
                            $className = 'RbMenu'.Tools::ucfirst(Tools::strtolower($this->fields['form']['connect_to']));
                        }
    ...
    public function update($null_value=false)
    {
        ...
        if (get_class($this)=='RbMegaMenu' && $this->enabled_vertical) {
            $columns= Db::getInstance()->executeS(
                ...
                AND id_tab
                not in (SELECT id_tab FROM '._DB_PREFIX_.'rbthememenu_tab where id_menu ='.(int)$this->id.')'
            );

            if ($columns) {
                $id_tab= Db::getInstance()->getValue(
                    'SELECT id_tab FROM
                    '._DB_PREFIX_.'rbthememenu_tab
                    WHERE id_menu='.(int)$this->id
                );
                ...
                foreach ($columns as $column) {
                    Db::getInstance()->execute(
                        'UPDATE '._DB_PREFIX_.'rbthememenu_column
                        SET id_tab = "'.(int)$id_tab.'"
