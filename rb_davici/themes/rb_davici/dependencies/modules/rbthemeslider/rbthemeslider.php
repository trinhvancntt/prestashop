...
if (!defined('_RB_VERSION_')) {
    define('_RB_VERSION_', '1.0.0');
}
...
$currentFolder = _PS_MODULE_DIR_ . 'rbthemeslider';
require_once $currentFolder . '/rbprestashoploader.php';
require_once $currentFolder . '/rbslider_admin.php';

class Rbthemeslider extends Module implements WidgetInterface
{
    public static $psdb;
    public static $lang;
    public static $_url;
    ...
    public function __construct()
    {
        ...
        $this->description = $this->l('This is great module create slider for theme Prestashop!');
        $this->ps_versions_compliancy = array('min' => '1.6', 'max' => _PS_VERSION_);
        self::$lang = $this->getLang();

        $this->rbSliderJSON = array(
            'rb_lang' => array(
                ...
                'this_feature_only_if_activated' => $this->l('This feature is only available if you activate Slider Rbthemeslider for this installation'),
                ...
                'of_slider_rbthemeslider' => $this->l('of Slider Rbthemeslider to work.'),
                'slider_rbthemeslider_shortcode_creator' => $this->l('Slider Rbthemeslider Shortcode Creator'),
    ...
    public function getLang()
    {
        ...
        $str_var['Rbthemeslider_Sliders'] = $this->l('Rbthemeslider Sliders');
        ...
        $str_var['Update_rb_Slider_Plugin'] = $this->l('Update Rbthemeslider Slider Plugin');
        $str_var['Update_rb_Slider_Plugin_desc'] = $this->l('To update the slider please show the slider install package. The files will be overwriten');
        $str_var['File_example'] = $this->l('File example: rbslider.zip');
        $str_var['Choose_update_file'] = $this->l('Choose the update file:');
        $str_var['No_Sliders_Found'] = $this->l('No Sliders Found');
        $str_var['Rbthemeslider_Slider_Temp'] = $this->l('Rbthemeslider Slider Templates');
        ...
        $str_var['more_google'] = $this->l('To add more google fonts please read <a target="_blank" href="http://codecanyon.net/item/slider-Rbthemeslider-responsive-wordpress-plugin/2751380/faqs/15268"> this tutorial </a> ');
        ...
        $str_var['Putting_javascript_conflicts'] = $this->l('Putting the js to body (in addition to head) is good for fixing some javascript conflicts of type: TypeError: tpj(\'#rb_slider_1_1\').show().Rbthemeslider is not a function');
        ...
        $str_var['Edit_Slide'] = $this->l('Edit Slide');
        $str_var['Preview_Slide'] = $this->l('Preview Slide');
        $str_var['New_Post'] = $this->l('<i class=\'rbicon-pencil-1\'></i>New Post');
        ...
        $str_var['Off'] = $this->l('off');
        $str_var['RbSlider_libraries'] = $this->l('Include RbSlider libraries globally');
        $str_var['shortcode_exists'] = $this->l('Add css and js includes only on all pages. Id turned to off they will added to pages where the rb_slider shortcode exists only. This will work only when the slider added by a shortcode.');
        $str_var['Pages_RbSlider'] = $this->l('Pages to include RbSlider libraries');
        ...
        $str_var['rb_special_class'] = $this->l('Adds a unique class to the li of the Slide like class="rb_special_class" (add only the classnames, seperated by space)');
        $str_var['Class'] = $this->l('Class');
        $str_var['rb_special_id'] = $this->l('Adds a unique ID to the li of the Slide like id="rb_special_id" (add only the id)');
        $str_var['rb_special_attr'] = $this->l('Adds a unique Attribute to the li of the Slide like attr="rb_special_attr" (add only the attribute)');
    ...
    {
        return Module::getInstanceByName('rbthemeslider');
    }

    public static function rbSliderShortcode($args)
    {
        if (!class_exists('RbSliderFront')) {
            require_once dirname(__FILE__) . '/rbslider_front.php';

            new RbSliderFront(ABSPATH);
        }

        $sliderAlias = UniteFunctionsRb::getVal($args, 0);
        ob_start();
        $slider = RbSliderOutput::putSlider($sliderAlias);
    ...
    public function install()
    {
        ...
        $this->installModuleTab('Ajax', 'ajax', '');
        $this->installModuleTab('Navigation', 'navigation', '');
        $this->installModuleTab('Upload', 'upload', '');

        $gethooks = $this->getHook();
        $check = 1;
        ...
        require_once ABSPATH . "/rbslider_admin.php";

        $res = RbSliderAdmin::onActivate();
        new RbSliderAdmin(ABSPATH, false);
    ...
    public function uninstall()
    {
        ...
        RbSliderAdmin::deleteDBTables();
        Configuration::deleteByName('sds_rb_hooks');
        Configuration::deleteByName('tp-google-fonts');
        $this->clearStaticStyles();
        $this->uninstallModuleTab('setting');
        $this->uninstallModuleTab('ajax');
        $this->uninstallModuleTab('navigation');
        $this->uninstallModuleTab('upload');
    ...
    public function getContent()
    {
        ...
        var ajaxurl = "'. $this->context->link->getAdminLink('AdminRbthemesliderAjax') . '";</script>';

        require_once ABSPATH . "/rbslider_admin.php";

        ob_start();
        $productAdmin = new RbSliderAdmin(ABSPATH);
    ...
    public function hookdisplayBackOfficeHeader()
    {
        ...
        $ctl = $this->context->controller;
        $ctl->addCSS($css_url.'css/adminicon.css');
        UniteFunctionsPSRb::initStaticVars();

        if (Tools::getvalue('configure') == 'rbthemeslider') {
            Media::addJsDef($this->rbSliderJSON);
            ...
            Media::addJsDef(array(
                'rs_plugin_validated' => ($validated == 'true' ? true : false)
            ));
            ...
            Media::addJsDef(array('rbsliderbackoffice' => $backofficeurl));
            ...
            $ctl->addCSS($css_url.'css/codemirror/codemirror.css');
            $ctl->addCSS($css_url.'css/edit_layers.css');
            $ctl->addCSS($css_url.'css/thickbox.css');
            $ctl->addCSS($css_url.'rs-plugin/css/settings.css');
            $ctl->addCSS($css_url.'rs-plugin/css/static-captions.css');
            $g_urlContent = UniteFunctionsPSRb::getUrlContent();
            $dynamicadmincss = 'index.php?controller=AdminrbthemeslidersliderAjax&token=' .
            Context::getcontext()->controller->token . '&rbControllerAction=captions';
            ...
            return $this->display(__FILE__, 'views/templates/admin/rbthemeslider_admin.tpl');
        }
    }
    ...
    {
        $css_url = "{$this->_path}views/css/";
        $js_url = "modules/".$this->name."/views/js/rs-plugin/js/";
        $this->context->controller->addCSS($css_url.'rs-plugin/css/settings.css');

        if (file_exists($css_url.'rs-plugin/css/static-captions.css')) {
          $this->context->controller->addCSS($css_url.'rs-plugin/css/static-captions.css');
        }

        if (file_exists($css_url.'rs-plugin/css/dynamic-captions.css')) {
        ...
        $this->context->controller->registerJavascript(
            $this->name.'-tools', $js_url.'jquery.themepunch.tools.min.js',
            array(
                'position' => 'bottom',
                'priority' => 170
            )
        ...
        $this->context->controller->registerJavascript(
            $this->name.'-main', $js_url.'jquery.themepunch.rbthemeslider.min.js',
            array(
                'position' => 'bottom',
                'priority' => 171
            )
    ...
    {
        require_once ABSPATH . "/rbslider_front.php";

        $rbfront = new RbSliderFront(ABSPATH);
    ...
    {
        $rbfront = $this->_prehook();

        $sliders = self::$psdb->getResults(
            "SELECT * FROM " . self::$psdb->prefix . GlobalsRbSlider::TABLE_SLIDERS_NAME
        );
    ...
    {
        $cache_id = 'rbslider_front_' . $hookPosition;
        ...
        if (!Cache::isStored($cache_id)) {
            ...
            if (!empty($sliders)) {
                ...
                foreach ($sliders as $slider):
                    ...
                    if (@Rbthemeslider::getIsset($params->template) &&
                        $params->template != 'false'
                    ) {
                        continue;
                    } else {
                        if (@Rbthemeslider::getIsset($params->id_shop) &&
                            $params->id_shop != Shop::getContextShopID()
                        ) {
                            continue;
                        } else {
                            $tp = (array) $params;
                            if (@Rbthemeslider::getIsset($params->displayhook) &&
                                $params->displayhook === $hookPosition
                            ) {
                                RbSliderOutput::putSlider($slider->id, '');
                            } elseif (@Rbthemeslider::getIsset($tp['displayhook']) &&
                                $tp['displayhook'] === $hookPosition
                            ) {
                                RbSliderOutput::putSlider($slider->id, '');
                            }
                        }
    ...
    public function generateSliderById($id = 1)
    {
        ...
        $cache_id = 'rbslider_front_' . $id;

        if (!Cache::isStored($cache_id)) {
            ob_start();
            RbSliderOutput::putSlider($id, '');
            $content = ob_get_clean();
            Cache::store($cache_id, $content);
        }
    ...
    public function addAsTrusted()
    {
        if (defined('self::CACHE_FILE_TRUSTED_MODULES_LIST') == true) {
            if (@Rbthemeslider::getIsset($this->context->controller->controller_name) &&
    ...
    {
        return RbGlobalObject::getIsset($var);
    }
    ...
    {
        return self::checkedSelectedHelper(
            $selected,
            $current,
            $echo,
            'selected'
