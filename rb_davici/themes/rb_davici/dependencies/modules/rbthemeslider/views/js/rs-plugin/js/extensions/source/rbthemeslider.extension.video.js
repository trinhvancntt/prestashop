...
jQuery.extend(true,_R, {
	...
	prepareCoveredVideo : function(asprat,opt,nextcaption) {
		var ifr = nextcaption.find('iframe, video'),
			wa = asprat.split(':')[0],
			ha = asprat.split(':')[1],
			li = nextcaption.closest('.tp-rbslider-slidesli'),
					...
					events: {
						"onStateChange": function(event) {		
							...
							if (event.data == YT.PlayerState.PLAYING) {
								...
								opt.c.trigger('rbthemeslider.slide.onvideoplay',getVideoDatas(player,"youtube",_nc.data()));
								...
								if ((event.data!=-1 && event.data!=3)) {
									...
									opt.c.trigger('rbthemeslider.slide.onvideostop',getVideoDatas(player,"youtube",_nc.data()));
			...
			case "vimeo":
				...
				f.addEvent('ready', function(){	
					...
					f.addEvent('play', function(data) {							
						...
						punchgs.TweenLite.to(_nc.find('.tp-videoposter'),0.3,{autoAlpha:0,force3D:"auto",ease:punchgs.Power3.easeInOut});
						punchgs.TweenLite.to(_nc.find('iframe'),0.3,{autoAlpha:1,display:"block",ease:punchgs.Power3.easeInOut});							
						opt.c.trigger('rbthemeslider.slide.onvideoplay',getVideoDatas(f,"vimeo",_nc.data()));
						opt.videoplaying=true;
						addVidtoList(_nc,opt);
						if (pforv) 
							opt.c.trigger('stoptimer');
					...
					f.addEvent('finish', function(data) {
							...
							opt.c.trigger('rbthemeslider.slide.onvideostop',getVideoDatas(f,"vimeo",_nc.data())); 
							if (_nc.data('nextslideatend')==true)
								opt.c.rbnext();
...
var htmlvideoevents = function(_nc,opt,startnow) {
	...
	if (jvideo.attr('control') == undefined ) {
		if (_nc.find('.tp-video-play-button').length==0 && !_ISM)
			_nc.append('<div class="tp-video-play-button"><i class="rbicon-right-dir"></i><span class="tp-rbstop">&nbsp;</span></div>');
	...
	addEvent(video,"play",function() {
		...
		if (!pforv) {				
			opt.videoplaying=false;
			opt.c.trigger('starttimer');
			opt.c.trigger('rbthemeslider.slide.onvideostop',getVideoDatas(video,"html5",_nc.data()));
		} else {				
			opt.videoplaying=true;
			opt.c.trigger('stoptimer');
			opt.c.trigger('rbthemeslider.slide.onvideoplay',getVideoDatas(video,"html5",_nc.data()));				
		}
