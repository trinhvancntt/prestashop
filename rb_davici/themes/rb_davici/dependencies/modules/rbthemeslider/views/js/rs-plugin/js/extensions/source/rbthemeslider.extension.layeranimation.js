...
jQuery.extend(true,_R, {
	...
	animateSingleCaption : function(_nc,opt,offsetx,offsety,i,recalled,triggerforce) {
		...
		if ($progress<1 && _nc.data('outstarted') != 1 || staticdirection==2 || triggerforce) {			
			...
			var rberseanim = _nc.data('transform_in')!=undefined ? _nc.data('transform_in').match(/\(R\)/gi) : false;

			// BUILD ANIMATION LIBRARY AND HOVER ANIMATION
			if (!_nc.data('$anims') || triggerforce || rberseanim) {				
			...
			if (staticdirection != 2 || triggerforce) {
			  ...
			  newtl.eventCallback("onStart",function(){			  	
				...
				if (_nc.data('iframes'))
			  	...
			  	opt.c.trigger("rbthemeslider.layeraction",data)
			  });
			...
			if (($start!="sliderenter" || ($start=="sliderenter" && opt.overcontainer)) && (staticdirection==-1 || staticdirection==1 || triggerforce || (staticdirection==0 && $progress<1 && _nc.hasClass("rb-static-visbile"))))				
...
var staticLayerStatus = function(_nc,opt,dir,dontmod) {
	...
	if (_nc.hasClass("tp-static-layer")) {	
		var s = parseInt(_nc.data('startslide'),0),
			e = parseInt(_nc.data('endslide'),0),
			pi = opt.c.find('.processing-rbslide').index(),
			ai = pi!=-1 ? pi : opt.c.find('.active-rbslide').index();
		...
		if (dir==="in") {				
			...
			if (!_nc.hasClass("rb-static-visbile")) {
				// IF ITEM SHOULD BECOME VISIBLE			
					
				if ((s<=ai && e>=ai) || (s == ai) || (e == ai)){						
					if (!dontmod) {
						_nc.addClass("rb-static-visbile");
						_nc.removeClass("rb-static-hidden");
					}
					a = 1;
				} else 
					a = 0;
			...
			if (_nc.hasClass("rb-static-visbile")) {					
				if ((s > ai) ||
					...
					a = 2;
					if (!dontmod) {
						_nc.removeClass("rb-static-visbile");
						_nc.addClass("rb-static-hidden");
					}
				} else {
					a = 0;
				}
