...
jQuery.extend(true,_R, {
	checkForParallax : function(container,opt) {
		...
		if (_.type=="mouse" || _.type=="scroll+mouse" || _.type=="mouse+scroll" || _.type=="3D" || _.type=="3d") {
			...
			container.mouseenter(function(event) {
				var currslide = container.find('.active-rbslide'),
			...
			container.on('mousemove.hoverdir, mouseleave.hoverdir, trigger3dpath',function(event,data) {				
				var currslide = data && data.li ? data.li : container.find('.active-rbslide');
				...
				if (_.type=="3D" || _.type=="3d") {
					var sctor = '.tp-rbslider-slidesli .dddwrapper, .dddwrappershadow, .tp-rbslider-slidesli .dddwrapper-layer';
					if (opt.sliderType==="carousel") sctor = ".tp-rbslider-slidesli .dddwrapper, .tp-rbslider-slidesli .dddwrapper-layer";
					opt.c.find(sctor).each(function() {										
						...
						if (li.hasClass("active-rbslide") || opt.sliderType!="carousel")
	...
	scrollHandling : function(opt) {	
		...
		if (_.type!="3d" && _.type!="3D") {
			...
			opt.c.find('.tp-rbslider-slidesli .slotholder, .tp-rbslider-slidesli .rs-background-video-layer').each(function() {			
				var t = jQuery(this),
