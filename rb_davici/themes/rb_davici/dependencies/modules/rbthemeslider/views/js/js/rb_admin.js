...
*/
var RbSliderAdmin = new function(){
	...
	var initSaveSliderButton = function(ajaxAction){
		...
		jQuery("#button_save_slider,#button_save_slider_t").click(function(){
			//collect data
			var data = {
				params: RbSliderSettings.getSettingsObject("form_slider_params"),
				main: RbSliderSettings.getSettingsObject("form_slider_main"),
				template: jQuery('#rbslider_template').val() //determinate if we are a template slider or not
			};
			
			//add slider id to the data
			if(ajaxAction == "update_slider"){
				data.sliderid = jQuery("#sliderid").val();
				data.params.custom_css = rb_cm_custom_css.getValue();
				data.params.custom_javascript = rb_cm_custom_js.getValue();

				UniteAdminRb.setAjaxLoaderID("loader_update, #loader_update_t");
				UniteAdminRb.setAjaxHideButtonID("button_save_slider,button_save_slider_t");
				UniteAdminRb.setSuccessMessageID("update_slider_success,#update_slider_success_t");

			}

			UniteAdminRb.ajaxRequest(ajaxAction, data);
		});
	}
	...
	var updateShortcode = function(){
		...
		var shortcode = '[rb_slider alias="'+alias+'"]';
		if(alias == "")
			shortcode = rb_lang.wrong_alias;
	...
	var initSliderViewCommon = function(){
		 ...
		 jQuery('input[name=facebook-page-url]').change(function(){
			if(jQuery("select[name='facebook-type-source']").val()=='album'){
				...
				if(jQuery('input[name=facebook-page-url]').val()!=""){
					UniteAdminRb.ajaxRequest("get_facebook_photosets", data,function(response){
		...
		jQuery('input[name=flickr-user-url],select[name=flickr-type]').change(function(){
			if(jQuery("select[name=flickr-type]").val()=='photosets'){
				if(jQuery('input[name=flickr-user-url]').val()!="" && jQuery('input[name=flickr-api-key]').val()!=""){
					...
					UniteAdminRb.ajaxRequest("get_flickr_photosets", data, function(response){
		...
		jQuery('input[name=youtube-channel-id]').change(function(){
			if(jQuery("select[name='youtube-type-source']").val()=='playlist'){
				...
				if(jQuery('input[name=youtube-channel-id]').val()!=""){
					UniteAdminRb.ajaxRequest("get_youtube_playlists", data,function(response){
		...
		jQuery('body').on('click', 'input[name="loop_slide"]', function(){
			...
			if(jQuery(this).val() == 'noloop'){
				...
				UniteAdminRb.showInfo({type: 'info', hideon: '', event: '', content: rb_lang.nav_bullet_arrows_to_none, hidedelay: 3});
			}
		});
		...
		jQuery('#enable_advanced_sizes').click(function(){
			jQuery('.rb-advanced-sizes-wrap').show();
			jQuery('#rb-enable-advanced-sizes').hide();
			jQuery('input[name="advanced-responsive-sizes"]').val('true');
			jQuery('.rb-desktop-naming').html('Desktop');
		...
		jQuery('#disable_advanced_sizes').click(function(){
			jQuery('.rb-advanced-sizes-wrap').hide();
			jQuery('#rb-enable-advanced-sizes').show();
			jQuery('input[name="advanced-responsive-sizes"]').val('false');
			jQuery('.rb-desktop-naming').html('');
		});
	}
	...
	**/
	...
	this.initEditSliderView = function(){
		...
		jQuery("#button_delete_slider, #button_delete_slider_t").click(function(){
			...
			if(confirm(rb_lang.really_want_to_delete+" '"+jQuery("#title").val()+"' ?") == false)
				return(true);

			var data = {sliderid: jQuery("#sliderid").val()}

			UniteAdminRb.ajaxRequest("delete_slider" ,data);
		});
		...
		jQuery("#button_replace_url").click(function(){
			if(confirm(rb_lang.sure_to_replace_urls) == false)
				return(false);
			...
			UniteAdminRb.setAjaxLoaderID("loader_replace_url");
			UniteAdminRb.setAjaxHideButtonID("button_replace_url");
			UniteAdminRb.setSuccessMessageID("replace_url_success");

			UniteAdminRb.ajaxRequest("replace_image_urls" ,data);
		});
		...
		jQuery('#reset_slide_button').click(function(){
			...
			if(confirm(rb_lang.set_settings_on_all_slider) == false)
				return(false);
			...
			UniteAdminRb.ajaxRequest('reset_slide_settings', data);
		});
		...
		jQuery('.tp-moderncheckbox').each(function(){
			RbSliderSettings.onoffStatus(jQuery(this));
		});
	...
	var updateSlidesOrder = function(sliderID){
		...
		UniteAdminRb.ajaxRequest("update_slides_order" ,data,function(){
	...
	this.initNewsletterRoutine = function(){
		...
		jQuery('#subscribe-to-newsletter').click(function(){
			...
			UniteAdminRb.ajaxRequest("subscribe_to_newsletter", data); //, '#subscribe-to-newsletter'
		});
		...
		jQuery('#unsubscribe-to-newsletter').click(function(){
			...
			UniteAdminRb.ajaxRequest("unsubscribe_to_newsletter", data); //, '#unsubscribe-to-newsletter'
		});
	...
	this.initSlidersListView = function(){
		...
		jQuery('body').on('click', '.rs-reload-shop', function(){
			// showWaitAMinute({fadeIn:300,text:rb_lang.please_wait_a_moment});
			UniteAdminRb.ajaxRequest("update_shop", {});
		 ...
		 **/
		jQuery('body').on('click', '.template_slider_item_import .icon-install_slider', function(){
			
                        var $this = jQuery(this).closest('.template_slider_item_import');
                        
			if($this.hasClass('deny_download')){
                                alert(rb_lang.this_template_requires_version+' '+$this.data('versionneed')+' '+rb_lang.of_slider_rbthemeslider);
				return false;
			}
			
			//modify the dialog with some informations
			jQuery('.rs-zip-name').text($this.data('zipname'));
			jQuery('.rs-uid').val($this.data('uid'));
				...
				buttons:{
					...
					"Online":function(){
						if(rs_plugin_validated){
							...
							alert(rb_lang.this_feature_only_if_activated);
						}
					}
				}
			});
			...
			jQuery('#dialog_duplicate_slider').dialog({
				...
				buttons:{
					...
					'Import':function(){
						...
						UniteAdminRb.ajaxRequest('duplicate_slider', {sliderid:slider_id,title:jQuery('#rs-duplicate-animation').val()}, function(response){
							jQuery('#close-template').click();
						});
					}
				}
		...
		jQuery(".input_import_slider").change(function(){
			if(jQuery(this).val() !== ''){
				jQuery('.rb-import-slider-button').show();
			}else{
				jQuery('.rb-import-slider-button').hide();
			}
		});
		...
		jQuery(".button_delete_slider").click(function(){
			...
			if(confirm(rb_lang.really_want_to_delete+" '"+sliderTitle+"' ?") == false)
				return(false);

			UniteAdminRb.ajaxRequest("delete_slider_stay" ,{sliderid:sliderID}, function(){
			...
			jQuery('#dialog_duplicate_slider').dialog({
				...
				buttons:{
					...
					'Duplicate':function(){
						...
						UniteAdminRb.ajaxRequest('duplicate_slider' ,{sliderid:sliderID,title:jQuery('#rs-duplicate-animation').val()}, function(response){
		...
		jQuery(".rb-toogle-fav").click(function(){
			var sliderID = this.id.replace('reg-toggle-id-','');
			UniteAdminRb.ajaxRequest('toggle_favorite' ,{id:sliderID},function(){
		...
		jQuery(".export_slider_standalone").click(function(){
			...
			var useDummy = false;//jQuery('input[name="export_dummy_images"]').is(':checked');
			var urlAjaxExport = ajaxurl+"?action="+g_uniteDirPlagin+"_ajax_action&client_action=preview_slider&only_markup=true&dummy="+useDummy;
			urlAjaxExport += "&sliderid=" + sliderID;
			location.href = urlAjaxExport;
	...
	t.initSlidesListViewPosts = function(sliderID){
		...
		jQuery("#select_sortby").change(function(){
			...
			UniteAdminRb.ajaxRequest("update_posts_sortby" ,data,function(){
		...
		jQuery(".button_delete_slide").click(function(){
			...
			UniteAdminRb.ajaxRequest("delete_slide" ,data);
		});
	...
	var initSlideListGlobals = function(sliderID){
		...
		jQuery("#list_slides .icon_state").click(function(){
			...
			objIcon.hide();
			objLoader.show();
			UniteAdminRb.ajaxRequest("toggle_slide_state" ,data,function(response){
				objIcon.show();
				objLoader.hide();
		...
		jQuery(".col-image .slide_image").click(function(){
			...
			UniteAdminRb.openAddImageDialog(g_messageChangeImage,function(urlImage,imageID){
				var data = {slider_id:sliderID,slide_id:slideID,url_image:urlImage,image_id:imageID};
				UniteAdminRb.ajaxRequest("change_slide_image" ,data);
			});
		}).tipsy({
			gravity:"s",
			delayIn: 70
	...
	t.initSlidesListView = function(sliderID){
		...
		jQuery("#button_new_slide, #button_new_slide_top").click(function(){
			...
			UniteAdminRb.openAddImageDialog(dialogTitle, function(obj){
				var data = {sliderid:sliderID,obj:obj};                                
				UniteAdminRb.ajaxRequest("add_slide" ,data);
		...
		jQuery(".button_duplicate_slide").click(function(){
			...
			UniteAdminRb.ajaxRequest("duplicate_slide" ,data);
		});
		...
		jQuery(".button_copy_slide").click(function(){
			...
			buttons[textUpdate] = function(){
				...
				UniteAdminRb.ajaxRequest("copy_move_slide" ,data);
				jQuery(this).dialog("close");
			};
	...
	t.saveEditSlide = function(slideID, isDemo){
		...
		var layers = UniteLayersRb.getLayers();
		...
		if(!isDemo){ //demo means static captions. This has
			data.params = RbSliderSettings.getSettingsObject("form_slide_params");
			...
			console.log(data.params);
				if((typeof(data.params.image_id) === 'undefined' || parseInt(data.params.image_id) == 0 || data.params.image_id == '') && ((typeof(data.params.image_url) === 'undefined') || data.params.image_url == '')){
					alert(rb_lang.cover_image_needs_to_be_set);
					return false;
				}
			}
		...
		if(!isDemo){
			UniteAdminRb.setAjaxHideButtonID("button_save_slide,button_save_slide-tb");
			UniteAdminRb.setAjaxLoaderID("loader_update");
			UniteAdminRb.setSuccessMessageID("update_slide_success");
			UniteAdminRb.ajaxRequest("update_slide", data);
		}else{
			UniteAdminRb.setAjaxHideButtonID("button_save_static_slide,button_save_static_slide-tb");
			UniteAdminRb.setAjaxLoaderID("loader_update");
			UniteAdminRb.setSuccessMessageID("update_slide_success");
			UniteAdminRb.ajaxRequest("update_static_slide", data);
		}
	}
	...
	this.initEditSlideView = function(slideID,sliderID,is_static){
		...
		jQuery('body').on('click', '.rs-reload-shop', function(){
			if(confirm(rb_lang.unsaved_data_will_be_lost_proceed)){
                            showWaitAMinute({fadeIn:300,text:rb_lang.please_wait_a_moment});
                            UniteAdminRb.ajaxRequest("update_shop", {});
                            location.href = window.location.href+'&update_shop';
			}
		...
		jQuery("#link_add_slide").click(function(){
			...
			UniteAdminRb.ajaxRequest("add_slide_fromslideview", data);
		});
		
		//add bulk slide top link
		jQuery("#link_add_bulk_slide").click(function(){
			UniteAdminRb.openAddImageDialog(rb_lang.add_bulk_slides, function(obj){
				var data = {sliderid:sliderID,obj:obj};
				UniteAdminRb.ajaxRequest("add_bulk_slide", data);
		...
		jQuery('body').on('click', '.template_item', function(){			
			if(confirm(rb_lang.unsaved_data_will_be_lost_proceed)){
				...
				UniteAdminRb.ajaxRequest('copy_slide_to_slider', data, function(){
					jQuery('#close-template').click();
				});
			}
		});
		...
		jQuery("#button_change_image").click(function(){
			...
			UniteAdminRb.openAddImageDialog(rb_lang.select_slide_img,function(urlImage,imageID){
		...
		jQuery('.button_change_video').click(function(){
			...
			UniteAdminRb.openAddVideoDialog(rb_lang.select_slide_video,function(urlVideo,videoID){
		...
		jQuery("#link_hide_options").click(function(){
			...
			if(jQuery("#slide_params_holder").is(":visible") == true){
				...
				jQuery(this).text(rb_lang.show_slide_opt).addClass("link-selected");
			}else{
				jQuery("#slide_params_holder").show("slow");
				jQuery(this).text(rb_lang.hide_slide_opt).removeClass("link-selected");
			}
		...
		UniteAdminRb.setColorPickerCallback(function(){
			var bgType = jQuery(".bgsrcchanger[name=background_type]:checked").val();
			if(bgType == "solid"){
				var bgColor = jQuery("#slide_bg_color").val();
				jQuery(".slotholder .tp-bgimg").css("background-color",bgColor);
		...
		jQuery('body').on('click', '.slide-remove', function(){
			...
			UniteAdminRb.ajaxRequest(do_request, data, function(response){
				curlayer.remove();
			});
		});
		...
		jQuery('body').on('click', '.slide-hero-unpublished.pubclickable', function(){
			...
			UniteAdminRb.ajaxRequest("toggle_hero_slide", data,function(response){
		...
		jQuery('body').on('click', '.slide-duplicate', function(){
			...
			var data = {slideID:slideID,sliderID:sliderID};
			UniteAdminRb.ajaxRequest("duplicate_slide_stay", data, function(response){
				objButton.show();
				...
				if(response.success == true){
					...
					if(new_ele.hasClass('selected')){ //current slide duplicated
						...
						jQuery('<a href="#" class="slide-link-toolbar-button slide-moveto"><span class=""><i class="eg-icon-forward"></i><span>'+rb_lang.copy_move+'</span></span></a>').insertAfter(new_ele.find('.slide-remove'));
					}
					
					var ahref = new_ele.find('a').first().attr('href');
					if(ahref == 'javascript:void(0)'){                                                
						ahref = rbsliderbackoffice+'&view=slide&id='+ response.id;
					}else{
		...
		jQuery('body').on('click', '.slide-moveto', function(){
			...
			buttons[textUpdate] = function(){
				...
				UniteAdminRb.ajaxRequest("copy_move_slide_stay", data, function(response){
					objButton.show();
		...
		jQuery('body').on('click', '.slide-add-as-template', function(){
			...
			if(confirm(rb_lang.unsaved_changes_will_not_be_added)){
				var cont = jQuery(this).closest('li');
				var title = prompt(rb_lang.please_enter_a_slide_title, cont.find('.slidetitleinput').val());
				...
				if (title != null) {
					...
					UniteAdminRb.ajaxRequest('add_slide_to_template', data, function(response){
						objButton.show();
		...
		jQuery("#rb_lang_list").delegate(".icon_slide_lang, .icon_slide_lang_add","click",function(event){

			event.stopPropagation()
			var pos = UniteAdminRb.getAbsolutePos(this);
		...
		jQuery("#slides_langs_float li a").click(function(){
			...
			UniteAdminRb.ajaxRequest("slide_lang_operation", data,function(response){
		...
		RbSliderSettings.onoffStatus(jQuery('input[name="stream_do_cover"]'));
		RbSliderSettings.onoffStatus(jQuery('input[name="stream_do_cover_both"]'));
		RbSliderSettings.onoffStatus(jQuery('input[name="kenburn_effect"]'));
		RbSliderSettings.onoffStatus(jQuery('input[name="save_performance"]'));
		RbSliderSettings.onoffStatus(jQuery('input[name="video_force_cover"]'));
		RbSliderSettings.onoffStatus(jQuery('input[name="video_nextslide"]'));
		RbSliderSettings.onoffStatus(jQuery('input[name="video_allowfullscreen"]'));
		RbSliderSettings.onoffStatus(jQuery('input[name="video_force_rewind"]'));
		RbSliderSettings.onoffStatus(jQuery('input[name="video_mute"]'));
		RbSliderSettings.onoffStatus(jQuery('input[name="thumb_for_admin"]'));
	...
	var openPreviewSlideDialog = function(slideID,useParams){
		...
		if(useParams == true){
			objData.params = RbSliderSettings.getSettingsObject("form_slide_params"),
			...
			objData.layers = UniteLayersRb.getLayers();
	...
	var initGlobalCssEditor = function(){
		...
		jQuery('#button_edit_css_global').click(function(){
			//if(!UniteLayersRb.getLayerGeneralParamsStatus()) return false; //false if fields are disabled

			jQuery("#css-static-accordion").accordion({ active: 1 });

			UniteAdminRb.ajaxRequest("get_static_css","",function(response){
				...
				if(g_codemirrorCssStatic != null)
					...
					setTimeout('RbSliderAdmin.setCodeMirrorStaticEditor()',500);
				}
			});

			UniteAdminRb.ajaxRequest("get_dynamic_css","",function(response){
				var cssData = response.data;
				...
				if(g_codemirrorCssDynamic != null)
					...
					setTimeout('RbSliderAdmin.setCodeMirrorDynamicEditor()',500);
				}
			});
			...
			jQuery("#css_static_editor_wrap").dialog({
				modal:true,
				resizable:false,
				title:rb_lang.global_styles_editor,
				minWidth:700,
				minHeight:500,
				closeOnEscape:true,
				open:function () {
					...
					.find(".ui-button").each(function(i) {
					   ...
					   if (i==1) cl="rbgreen";
					   if (i==2) cl="revred";
					   jQuery(this).addClass(cl).addClass("button-primary").addClass("rb-uibuttons");
				   })
				},
				buttons:{
					Save:function(){
						if(!confirm(rb_lang.really_update_global_styles)){
							return false;
						}
						...
						UniteAdminRb.ajaxRequest("update_static_css",data,function(response){
							if(g_codemirrorCssStatic != null)
