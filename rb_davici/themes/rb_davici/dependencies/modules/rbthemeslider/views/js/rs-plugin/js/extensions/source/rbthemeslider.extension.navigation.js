...
jQuery.extend(true,_R, {
	...
	// MANAGE THE NAVIGATION
	 createNavigation : function(container,opt) {
		...
		container.bind('rbthemeslider.slide.onafterswap rbthemeslider.nextslide.waiting',function() {		
			
			//cp.find('.tp-bullet, .tp-thumb, .tp-tab').removeClass("selected");			
			
			var si = container.find(".next-rbslide").length==0 ? container.find(".active-rbslide").data("index") : container.find(".next-rbslide").data("index");			
...
var initMouseScroll = function(container,opt) {				
	...
	container.on('mousewheel DOMMouseScroll', function(e) {							
			var res = normalizeWheel(e.originalEvent),		
				asi = container.find('.tp-rbslider-slidesli.active-rbslide').index(),
				psi = container.find('.tp-rbslider-slidesli.processing-rbslide').index(),
...
var swipeAction = function(container,opt,vertical) {	
	...
	SwipeOn.swipetp({			
		...
		swipeStatus:function(event,phase,direction,distance,duration,fingerCount,fingerData) {			
			...
			var withinslider = isme('rb_slider_wrapper',container,event),
...
var initArrows = function(container,o,opt) {
	...
	ra.click(function() { opt.sc_indicator="arrow"; opt.sc_indicator_dir = 0;container.rbnext();});
	la.click(function() { opt.sc_indicator="arrow"; opt.sc_indicator_dir = 1;container.rbprev();});
...
var setNavElPositions = function(el,o) {
	...
	var wrapper =  
		el.closest('.tp-simpleresponsive').length>0 ? 
			...
			el.closest('.tp-rbslider-mainul').length>0 ? 
				el.closest('.tp-rbslider-mainul')  : 
				el.closest('.rb_slider_wrapper').length>0  ? 
				el.closest('.rb_slider_wrapper'):
				el.parent().find('.tp-rbslider-mainul'),
		ww = wrapper.width(),
		wh = wrapper.height();	
...
var addBullet = function(container,o,li,opt) {
	...
	b.click(function() {
		...
		container.rbcallslidewithid(linkto);
