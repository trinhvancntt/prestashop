			...
			{if ( ((Configuration::get('PS_ATTRIBUTE_CATEGORY_DISPLAY') == 1) && (!empty($product.main_variants))) || (Configuration::get('PS_ATTRIBUTE_CATEGORY_DISPLAY') == 1 || empty($product.main_variants)))}
				{block name='product_quantity'}
					<div class="product-quantity">
						<div class="add">
							<button class="btn rb-btn-product add-to-cart" title="{l s='Add to cart' d='rbthemefunction'}" data-button-action="add-to-cart" type="submit">
								<i class="icon-bag"></i>
								<span class="icon-title">{l s='Add To Cart' d='Shop.Theme.Global'}</span>
							</button>
