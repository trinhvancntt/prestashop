...
{block name="other_input"}
	{if $key eq 'selects'}
		<div class="margin-form">
			<h3>{l s='Posts:' mod='rbthemeblog'}</h3>
					...
					<td style="padding-left:20px;">
						<select multiple id="select_left">
							{foreach from=$field.posts_unselected item='post'}
								<option value="{$post.id_rbblog_post|intval}">
						...
						<span class="hint" name="help_box">
							{l s='Double click to move the item to another column' mod='rbthemeblog'}
							<span class="hint-pointer">&nbsp;</span>
						</span>

						<br /><br />
						<a href="#" id="move_to_right" class="multiple_select_add">
							{l s='Add' mod='rbthemeblog'} &gt;&gt;
						</a>
					</div>
					</td>
					<td>
						<select multiple id="select_right" name="posts[]">
							{foreach from=$field.posts item='post'}
								<option selected="selected" value="{$post.id_rbblog_post|intval}">
									{$post.title|escape:'html':'UTF-8'}
								</option>
							{/foreach}
						</select>
