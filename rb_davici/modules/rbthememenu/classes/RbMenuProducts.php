...
class RbMenuProducts
{
    private static $is17 = true;
    private $nProducts = 1;
    ...
    public function getBestSellers($count = false)
    {
        ...
        if (($bestSales = ProductSale::getBestSales(
            ...
            $use_tax = (Product::getTaxCalculationMethod((isset($this->context->customer->id) &&
                $this->context->customer->id? (int)$this->context->customer->id : null)) != PS_TAX_EXC
            );
    ...
    public function getHomeFeatured($count = false)
    {
        ...
        if ($count) {
            return $category->getProducts(
                ...
                false,
                1,
                ($this->context->controller->controller_type != 'admin'? true : false),
                $this->context
            );
        }
    ...
    public function getSpecialProducts($count = false)
    {
        ...
        $pricesDrops = self::getPricesDrop(
            (int)$this->context->language->id,
            $this->Page,
            $this->nProducts,
            false,
    ...
    public static function getPricesDrop(
        ...
        if (Group::isFeatureActive()) {
            ...
            $sql_groups = ' AND EXISTS(SELECT 1 FROM `'._DB_PREFIX_.'category_product` cp JOIN `'
            ._DB_PREFIX_.'category_group` cg ON (cp.id_category = cg.id_category AND cg.`id_group` '.
            (count($groups) ? 'IN ('.implode(',', $groups).')' : '= 1').') WHERE cp.`id_product` = p.`id_product`)';
        }
        ...
        if ($count) {
            return Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue(
                ...
                '.((!$beginning && !$ending) ? 'AND p.`id_product` IN('.((is_array($tab_id_product) &&
                    count($tab_id_product)) ? implode(', ', $tab_id_product) : 0).')' : '').'
                '.$sql_groups
            );
        }
    ...
    protected static function _getProductIdByDate(
        $beginning,
        $ending,
        Context $context = null,
