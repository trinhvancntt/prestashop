...
class RbMenuConfig
{
    ...
    public function __construct()
    {
        ...
        $this->setFields(Rbthememenu::$configs);
    }
    ...
    public function renderForm()
    {
        ...
        if ($configs) {
            foreach ($configs as $key => $config) {
                $confFields = array(
                    ...
                    'img_del_link' => isset($config['type']) && $config['type']=='file' && Configuration::get($key)!='' && @file_exists(dirname(__FILE__).'/../views/img/upload/'.Configuration::get($key)) ? $helper->module->baseAdminUrl().'&deleteimage='.$key.'&itemId=0&rb_object=Rb_'.Tools::ucfirst($fields_form['form']['name']) : false,
                );
        ...
        $helper->identifier = 'rb_form_'.$this->fields['form']['name'];
        ...
        $helper->tpl_vars = array(
            ...
            'rb_clear_cache_url' => $helper->module->baseAdminUrl(),
            'reset_default' => true,
        );
    ...
    public function saveData()
    {
                    ...
                    if (isset($config['lang']) && $config['lang']) {
                        ...
                        foreach ($languages as $lang) {
                            if ($config['type']=='switch')
                                ...
                                $valules[$lang['id_lang']] = trim(Tools::getValue($key.'_'.$lang['id_lang'])) ?
                                trim(Tools::getValue($key.'_'.$lang['id_lang'])) : trim(Tools::getValue($key.'_'.$id_lang_default));
                        }
                        ...
                        if (isset($_FILES[$key]['tmp_name']) &&
                            ...
                            $imageName = @file_exists(dirname(__FILE__).'/../views/img/upload/'.
                            Tools::strtolower($_FILES[$key]['name'])) ? $salt.'-'.Tools::strtolower($_FILES[$key]['name']) :
                            Tools::strtolower($_FILES[$key]['name']);
                        ...
                        isset($config['tree']['use_checkbox']) &&
                        $config['tree']['use_checkbox'] ||
        ...
        if (!$errors) {
            ...
            if (Configuration::get('rbthememenu_CACHE_ENABLED')) Rbthememenu::clearAllCache();
        }
    ...
    public function getConfig()
    {
        ...
        if ($configs)
            foreach ($configs as $key => $config) {
                $data[$key] = isset($config['lang']) &&
                $config['lang'] ? Configuration::get($key, $this->context->language->id) : Configuration::get($key);
            }
