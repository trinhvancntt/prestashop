...
class RBSliderNavigation
{
    ...
    public function initById($nav_id)
    {
        if ((int) ($nav_id) == 0) {
            return false;
        }
        ...
        $row = $psdb->getRow(
            $psdb->prepare(
                "SELECT `id`, `handle`, `type`, `css`, `settings`
                FROM " . $psdb->prefix .
                RbSliderGlobals::TABLE_NAVIGATION_NAME . "
                WHERE `id` = %d",
                $nav_id
            ),
            ARRAY_A
     ...
     * @since: 5.0
     * */
    public static function getAllNavigationsShort()
    {
        $psdb = rbDbClass::rbDbInstance();

        $navigations = $psdb->getResults(
     ...
     * @since: 5.0
     * */
    public static function getAllNavigations($defaults = true, $raw = false)
    {
        $psdb = rbDbClass::rbDbInstance();

        $navigations = $psdb->getResults(
            "SELECT * FROM " . $psdb->prefix .
            RbSliderGlobals::TABLE_NAVIGATION_NAME,
            ARRAY_A
        );
        ...
        if ($raw == false) {
            foreach ($navigations as $key => $nav) {
                $navigations[$key]['css'] = preg_replace_callback(
                    '/\\"(.*)?\\"\;/',
                    function($m) {return "\"{$m[1]}\"";},
                    Tools::jsonDecode($navigations[$key]['css'],true)
                );

                $navigations[$key]['markup'] = Tools::jsonDecode(
                    $navigations[$key]['markup'],
                    true
                );

                if (@Rbthemeslider::getIsset($navigations[$key]['settings'])) {
                    $navigations[$key]['settings'] = Tools::jsonDecode(
                        $navigations[$key]['settings'],
                        true
                    );
                }
            }
        ...
        if ($defaults) {
            $def = self::getDefaultNavigations();

            if (!empty($def)) {
                if ($raw == false) {
                    foreach ($def as $key => $nav) {
                        $def[$key]['css'] = preg_replace_callback(
                            '/\\"(.*)?\\"\;/',
                            function($m){return "\"{$m[1]}\"";},
                            RbSliderBase::stripslashesDeep(
                                Tools::jsonDecode($def[$key]['css'], true)
                            )
                        );

                        $def[$key]['markup'] = RbSliderBase::stripslashesDeep(Tools::jsonDecode($def[$key]['markup'],true)
                        );
                    }

                    if (@Rbthemeslider::getIsset($def[$key]['settings'])) {
                        $def[$key]['settings'] = RbSliderBase::stripslashesDeep(Tools::jsonDecode($def[$key]['settings'], true)
                        );
                    }
                }
     ...
     * @since: 5.0
     * */
    public function createUpdateFullNavigation($data)
    {
        if (!empty($data) && is_array($data)) {
            $navigations = self::getAllNavigations();
            ...
            foreach ($data as $vals) {
                ...
                if (!@Rbthemeslider::getIsset($vals['markup']) ||
                    !@Rbthemeslider::getIsset($vals['css'])
                ) {
                    continue;
                }

                if (@Rbthemeslider::getIsset($vals['default']) &&
                    $vals['default'] == true
                ) {
                    continue;
                }

                if (@Rbthemeslider::getIsset($vals['id'])) {
                    if ((int) ($vals['id']) === 0) {
                        continue;
                    }
                ...
                if ($found == true) {
                    self::createUpdateNavigation($vals, $vals['id']);
                } else {
                    self::createUpdateNavigation($vals);
                }
            }
        }
     ...
     * @since: 5.0
     * */
    public static function createUpdateNavigation($data, $nav_id = 0)
    {
        $psdb = rbDbClass::rbDbInstance();

        if (@Rbthemeslider::getIsset($data['default']) && $data['default'] == true) {
            return false;
        }
        ...
        if ($nav_id > 0) {
            $response = $psdb->update(
                $psdb->prefix . RbSliderGlobals::TABLE_NAVIGATION_NAME,
                array(
                    'name' => $data['name'],
                    'handle' => sanitizeTitle($data['name']),
                    'markup' => Tools::jsonEncode($data['markup']),
                    'css' => Tools::jsonEncode($data['css']),
                    'settings' => Tools::jsonEncode($data['settings'])
                ),
     ...
     * @since: 5.0
     * */
    public function deleteNavigation($nav_id = 0)
    {
        $psdb = rbDbClass::rbDbInstance();

        if (!@Rbthemeslider::getIsset($nav_id) || (int) ($nav_id) == 0) {
            return $this->modules->l('Invalid ID');
        }

        $response = $psdb->query(
            "DELETE FROM `" . $psdb->prefix . RbSliderGlobals::TABLE_NAVIGATION_NAME . "`
            WHERE `id`='" . $nav_id . "';"
        );

        if ($response === false) {
            return $this->modules->l(
                'Navigation could not be deleted'
            );
        }
     ...
     * @since: 5.0
     * */
    public static function getDefaultNavigations()
    {
        $navigations = array();
     ...
     * @since: 5.0
     * */
    public static function translateNavigation($handle)
    {
        switch ($handle) {
            case 'round':
                $handle = 'hesperiden';
     ...
     * @since: 5.1.1
     * */
    public static function exportNavigation($nav_handle)
    {
        $navs = self::getAllNavigations(false, true);

        if (!is_array($nav_handle)) {
            $nav_handle = array($nav_handle => true);
        }
