...
class UniteSettingsRb
{
        ...
        foreach ($arrSettings as $key => $content) {
            switch (getType($content)) {
                ...
                case "array":
                    ...
                    $content = UniteFunctionsRb::convertStdClassToArray($content);
    ...
    {
        if (!@Rbthemeslider::getIsset($arrParams["items"])) {
    ...
    protected function getArrTypes()
    {
        ...
        foreach ($this->arrSettings as $setting) {
            ...
            if (!@Rbthemeslider::getIsset($arrTypesAssoc[$type])) {
                $arrTypes[] = $type;
            }
    ...
    public function getArrSettingNames()
    {
        ...
        foreach ($this->arrSettings as $setting) {
            $name = UniteFunctionsRb::getVal($setting, "name");
    ...
    public function getArrSettingNamesAndTitles()
    {
        ...
        foreach ($this->arrSettings as $setting) {
            $name = UniteFunctionsRb::getVal($setting, "name");
            $title = UniteFunctionsRb::getVal($setting, "text");
    ...
    {
        if (empty($text)) {
            UniteFunctionsRb::throwError("sap $name must have a text");
        }
    ...
    public function addSection($label, $name = "")
    {
        if (!empty($this->arrSettings) && empty($this->arrSections)) {
            UniteFunctionsRb::throwError(
                "You should add first section before begin to add settings. (section: $label)"
            );
        }

        if (empty($label)) {
            UniteFunctionsRb::throwError("You have some section without text");
        }
    ...
    protected function add(
        ...
        //validate name:
        if (@Rbthemeslider::getIsset($this->arrIndex[$name])) {
        ...
        //set datatype
        if (!@Rbthemeslider::getIsset($setting["datatype"])) {
    ...
    {
        UniteFunctionsRb::validateNotEmpty($control_item_name, "control parent");
        UniteFunctionsRb::validateNotEmpty($controlled_item_name, "control child");
        UniteFunctionsRb::validateNotEmpty($control_type, "control type");
        UniteFunctionsRb::validateNotEmpty($value, "control value");
        $arrControl = array();

        if (@Rbthemeslider::getIsset($this->arrControls[$control_item_name])) {
    ...
    public function startBulkControl($control_item_name, $control_type, $value)
    {
        $this->arrBulkControl = array(
            "control_name" => $control_item_name,
            "type" => $control_type,
            "value" => $value
        );
    }
    ...
    private function buildArrSettingsIndex()
    {
        ...
        foreach ($this->arrSettings as $key => $value) {
            if (@Rbthemeslider::getIsset($value["name"])) {
    ...
    public function setSettingsStateByControls()
    {
        foreach ($this->arrControls as $control_name => $arrControlled) {
            if (!@Rbthemeslider::getIsset($this->arrIndex[$control_name])) {
            ...
            foreach ($arrControlled as $controlled) {
                if (!@Rbthemeslider::getIsset($this->arrIndex[$controlled["name"]])) {
    ...
    private function checkAddBulkControl($name)
    {
        if (!empty($this->arrBulkControl)) {
            $this->addControl(
                $this->arrBulkControl["control_name"],
                $name,
                $this->arrBulkControl["type"],
                $this->arrBulkControl["value"]
            );
        }
    }
    ...
    private function getOptionsFromXMLField($field, $fieldName)
    {
        ...
        $options = UniteFunctionsRb::getVal($arrField, "option");
        ...
        foreach ($options as $option) {
            if (gettype($option) == "string") {
                UniteFunctionsRb::throwError("Wrong options type: " . $option . " in field: $fieldName");
            }

            $attribs = $option->attributes();
            $optionValue = (string) UniteFunctionsRb::getVal($attribs, "value");
            $optionText = (string) UniteFunctionsRb::getVal($attribs, "text");

            //validate options:

            UniteFunctionsRb::validateNotEmpty($optionValue, "option value");
            UniteFunctionsRb::validateNotEmpty($optionText, "option text");
    ...
    public function loadXMLFile($filepath)
    {
        if (!file_exists($filepath)) {
            UniteFunctionsRb::throwError("File: '$filepath' not exists!!!");
        }

        $obj = simplexml_load_file($filepath);

        if (empty($obj)) {
            UniteFunctionsRb::throwError("Wrong xml file format: $filepath");
        }
        ...
        foreach ($fieldsets as $fieldset) {
            ...
            $sapName = (string) UniteFunctionsRb::getVal($attribs, "name");
            $sapLabel = (string) UniteFunctionsRb::getVal($attribs, "label");

            if ($sapLabel != '' && @Rbthemeslider::getIsset(Rbthemeslider::$lang[$sapLabel])) {
                $sapLabel = Rbthemeslider::$lang[$sapLabel];
            }

            $sapIcon = (string) UniteFunctionsRb::getVal($attribs, "icon");
            UniteFunctionsRb::validateNotEmpty($sapName, "sapName");
            UniteFunctionsRb::validateNotEmpty($sapLabel, "sapLabel");
            ...
            foreach ($fields as $field) {
                ...
                $fieldType = (string) UniteFunctionsRb::getVal($attribs, "type");
                $fieldName = (string) UniteFunctionsRb::getVal($attribs, "name");
                $fieldLabel = (string) UniteFunctionsRb::getVal($attribs, "label");

                if ($fieldLabel != '' &&
                    @Rbthemeslider::getIsset(Rbthemeslider::$lang[$fieldLabel])
                ) {
                    $fieldLabel = Rbthemeslider::$lang[$fieldLabel];
                }

                $fieldDefaultValue = (string) UniteFunctionsRb::getVal($attribs, "default");
                ...
                UniteFunctionsRb::validateNotEmpty($fieldType, "type");

                //validate name
                if ($fieldType != self::TYPE_HR && $fieldType != self::TYPE_CONTROL &&
                    $fieldType != "bulk_control_start" && $fieldType != "bulk_control_end") {
                    UniteFunctionsRb::validateNotEmpty($fieldName, "name");
                }
                switch ($fieldType) {
                    case self::TYPE_CHECKBOX:
                        $fieldDefaultValue = UniteFunctionsRb::strToBool($fieldDefaultValue);
                    ...
                    case self::TYPE_CONTROL:
                        $parent = UniteFunctionsRb::getVal($arrParams, "parent");
                        $child = UniteFunctionsRb::getVal($arrParams, "child");
                        $ctype = UniteFunctionsRb::getVal($arrParams, "ctype");
                        $value = UniteFunctionsRb::getVal($arrParams, "value");
                    ...
                    case "codemirror":
                        ...
                        break;
                    default:
                        UniteFunctionsRb::throwError("wrong type: $fieldType");
                        break;
                }
            }
        }
    ...
    public function getArrTextFromAllSettings()
    {
        ...
        foreach ($this->arrSections as $section) {
            $arrSaps = UniteFunctionsRb::getVal($section, "arrSaps");
        ...
        foreach ($this->arrSettings as $setting) {
            ...
            $desc = UniteFunctionsRb::getVal($setting, "description");
            ...
            if (!empty($unit)) {
                if (!@Rbthemeslider::getIsset($arrUnits[$unit])) {
                    $arr[] = $unit;
                }
            ...
            $items = UniteFunctionsRb::getVal($setting, "items");

            if (!empty($items)) {
                foreach ($items as $item) {
                    if (!@Rbthemeslider::getIsset($arrUnits[$item])) {
                        $arr[] = $item;
                    }
    ...
    public function getSettingByName($name)
    {
        ...
        if (!empty($this->arrIndex)) {
            if (array_key_exists($name, $this->arrIndex) == false) {
                UniteFunctionsRb::throwError("setting $name not found");
            }
        ...
        //if no index
        foreach ($this->arrSettings as $setting) {
            $settingName = UniteFunctionsRb::getVal($setting, "name");
        ...
        UniteFunctionsRb::throwError("Setting with name: $name don't exists");
    }

    public function getSettingValue($name, $default = "")
    {
        $setting = $this->getSettingByName($name);
        $value = UniteFunctionsRb::getVal($setting, "value", $default);
        return($value);
    }
    ...
    public function updateArrSettingByName($name, $setting)
    {
        foreach ($this->arrSettings as $key => $settingExisting) {
            $settingName = UniteFunctionsRb::getVal($settingExisting, "name");
    ...
    public function setStoredValues($arrValues)
    {
        foreach ($this->arrSettings as $key => $setting) {
            ...
            $datatype = UniteFunctionsRb::getVal($setting, "datatype");
    ...
    public function getArrValues()
    {
        ...
        foreach ($this->arrSettings as $setting) {
            ...
            //remove lf
            if (@Rbthemeslider::getIsset($setting["remove_lf"])) {
                $value = str_replace("\n", "", $value);
                $value = str_replace("\r\n", "", $value);
            }
