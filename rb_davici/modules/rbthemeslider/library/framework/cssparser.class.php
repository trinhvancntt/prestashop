...
class UniteCssParserRb
{
    ...
    public static function parseDbArrayToCss($cssArray, $nl = "\n\r")
    {
        ...
        foreach ($cssArray as $id => $attr) {
            ...
            if (is_array($styles) || is_array($styles_adv)) {
                if (is_array($styles)) {
                    foreach ($styles as $name => $style) {
                        ...
                        if (!is_array($name) && @Rbthemeslider::getIsset($transparency[$name])) {
                            if (@Rbthemeslider::getIsset($styles[$transparency[$name]]) &&
                                $style !== 'transparent'
                            ) {
                                $style = RbSliderFunctions::hex2rgba($style, $styles[$transparency[$name]] * 100);
                            }
                        }

                        if (!is_array($name) && @Rbthemeslider::getIsset($check_parameters[$name])) {
                            $style = RbSliderFunctions::addMissingVal($style, $check_parameters[$name]);
                        }
            ...
            if (@Rbthemeslider::getIsset($setting['hover']) && $setting['hover'] == 'true') {
                ...
                if (is_array($hover) || is_array($hover_adv)) {
                    ...
                    if (is_array($hover)) {
                        foreach ($hover as $name => $style) {
                            ...
                            if (!is_array($name) && @Rbthemeslider::getIsset($transparency[$name])) {
                                if (@Rbthemeslider::getIsset($hover[$transparency[$name]]) &&
                                    $style !== 'transparent'
                                ) {
                                    $style = RbSliderFunctions::hex2rgba($style, $hover[$transparency[$name]] * 100);
                                }
                            }
                            if (!is_array($name) && @Rbthemeslider::getIsset($check_parameters[$name])) {
                                $style = RbSliderFunctions::addMissingVal($style, $check_parameters[$name]);
    ...
    {
        $db = new RbSliderDB();
        $styles = $db->fetch(RbSliderGlobals::$table_css, '', 'handle ASC');
        $arr = array('5.0' => array(), 'Custom' => array(), '4' => array());

        foreach ($styles as $style) {
            $setting = Tools::jsonDecode($style['settings'], true);

            if (!@Rbthemeslider::getIsset($setting['type'])) {
                $setting['type'] = 'text';
            }

            if (array_key_exists('version', $setting) && @Rbthemeslider::getIsset($setting['version'])) {
                $arr[Tools::ucfirst($setting['version'])][] = array(
                    'label' => trim(str_replace('.tp-caption.', '', $style['handle'])),
                    'type' => $setting['type']
                );
            }
        }
...
class RbSliderCssParser extends UniteCssParserRb
{
