...
class UniteFunctionsRb
{
    ...
    public static function filterArrFields($arr, $fields)
    {
        ...
        foreach ($fields as $field) {
            if (@Rbthemeslider::getIsset($arr[$field])) {
    ...
    public static function getPathInfo($filepath)
    {
        ...
        if (!@Rbthemeslider::getIsset($info["filename"])) {
    ...
    public static function writeFile($str, $filepath)
    {
        ...
        if (!is_writable(dirname($filepath))) {
            UniteFunctionsRb::throwError("Can't write file \"".$filepath."\", please change the permissions!");
        }
    ...
    private static function checkPHPVersion()
    {
        ...
        if ($version < 5) {
            throw new Exception(
            	"You must have php5 and higher in order to run the application. Your php version is: $version"
            );
        }
    }
    ...
    private static function validateGD()
    {
        if (function_exists('gd_info') == false) {
            throw new Exception(
            	"You need GD library to be available in order to run this application. Please turn it on in php.ini"
            );
        }
    }
    ...
    public static function getFileList($path, $ext = "")
    {
        ...
        foreach ($dir as $file) {
            ...
            if (!empty($ext)) {
                $info = pathinfo($file);
                $extension = UniteFunctionsRb::getVal($info, "extension");
    ...
    public static function trimArrayItems($arr)
    {
        if (gettype($arr) != "array") {
            UniteFunctionsRb::throwError("trimArrayItems error: The type must be array");
        }
    ...
    public static function normalizeDeviceSettings($obj, $enabled_devices, $return = 'obj', $set_to_if = array())
    {
        ...
        if ($enabled_devices['desktop'] == 'on') {
            if (!@Rbthemeslider::getIsset($obj->desktop) || $obj->desktop === '') {
        ...
        if ($enabled_devices['notebook'] == 'on') {
            if (!@Rbthemeslider::getIsset($obj->notebook) || $obj->notebook === '') {
        ...
        if ($enabled_devices['tablet'] == 'on') {
            if (!@Rbthemeslider::getIsset($obj->tablet) || $obj->tablet === '') {
        ...
        if ($enabled_devices['mobile'] == 'on') {
            if (!@Rbthemeslider::getIsset($obj->mobile) || $obj->mobile === '') {
    ...
    public static function getTransFromRgba($rgba, $in_percent = false)
    {
        ...
        if (count($temp) == 4) {
            return ($in_percent) ? preg_replace('/[^\d.]/', '', $temp[3]) :
            preg_replace('/[^\d.]/', "", $temp[3]) * 100;
        }
        return 100;
    }
    ...
    public static function getResponsiveSize($slider)
    {
        ...
        if ($adv_resp_sizes == true) {
            $width = $slider->slider->getParam("width", 1240, RbSlider::FORCE_NUMERIC);
            $width .= ','. $slider->slider->getParam("width_notebook", 1024, RbSlider::FORCE_NUMERIC);
            $width .= ','. $slider->slider->getParam("width_tablet", 778, RbSlider::FORCE_NUMERIC);
            $width .= ','. $slider->slider->getParam("width_mobile", 480, RbSlider::FORCE_NUMERIC);
            $height = $slider->slider->getParam("height", 868, RbSlider::FORCE_NUMERIC);
            $height .= ','. $slider->slider->getParam("height_notebook", 768, RbSlider::FORCE_NUMERIC);
            $height .= ','. (int)($slider->slider->getParam("height_tablet", 960, RbSlider::FORCE_NUMERIC));
            $height .= ','. (int)($slider->slider->getParam("height_mobile", 720, RbSlider::FORCE_NUMERIC));
            $responsive = (@Rbthemeslider::getIsset($arrValues['width'])) ? $arrValues['width'] : '1240';
            $def = (@Rbthemeslider::getIsset($arrValues['width'])) ? $arrValues['width'] : '1240';

            $responsive.= ',';

            if ($enable_custom_size_notebook == 'on') {
                $responsive.= (@Rbthemeslider::getIsset($arrValues['width_notebook'])) ? $arrValues['width_notebook'] : '1024';
                $def = (@Rbthemeslider::getIsset($arrValues['width_notebook'])) ? $arrValues['width_notebook'] : '1024';
            } else {
                $responsive.= $def;
            }
            ...
            if ($enable_custom_size_tablet == 'on') {
                $responsive.= (@Rbthemeslider::getIsset($arrValues['width_tablet'])) ? $arrValues['width_tablet'] : '778';
                $def = (@Rbthemeslider::getIsset($arrValues['width_tablet'])) ? $arrValues['width_tablet'] : '778';
            } else {
                $responsive.= $def;
            }
            ...
            if ($enable_custom_size_iphone == 'on') {
                $responsive.= (@Rbthemeslider::getIsset($arrValues['width_mobile'])) ? $arrValues['width_mobile'] : '480';
                $def = (@Rbthemeslider::getIsset($arrValues['width_mobile'])) ? $arrValues['width_mobile'] : '480';
            } else {
                $responsive.= $def;
            }
...
class RbSliderFunctions extends UniteFunctionsRb
{
