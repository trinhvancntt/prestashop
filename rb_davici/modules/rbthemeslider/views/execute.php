...
if ($thumb_pos === false ||
    preg_match('/\.{1,2}[\/|\\\]/', Tools::getValue('path_thumb')) !== 0 ||
    preg_match('/\.{1,2}[\/|\\\]/', Tools::getValue('path')) !== 0
)
    die('wrong path');
...
if (@Rbthemeslider::getIsset($info['extension']) &&
    !(Tools::getValue('action') &&
        Tools::getValue('action') == 'delete_folder') &&
    !in_array(Tools::strtolower($info['extension']),
        $ext)
)
    die('wrong extension');
