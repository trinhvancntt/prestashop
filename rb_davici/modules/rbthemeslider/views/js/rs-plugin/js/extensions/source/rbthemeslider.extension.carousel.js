...
jQuery.extend(true,_R, {
	...
	organiseCarousel : function(opt,direction,setmaind,unli) {	
		...
		if (unli) {
			opt.c.find('.next-rbslide').removeClass("next-rbslide");
			jQuery(_.slides[_.focused]).addClass("next-rbslide");
			opt.c.trigger("rbthemeslider.nextslide.waiting");
		}	
...
var defineCarouselElements = function(opt) {
	...
	// COLLECT SLIDES
	_.slides = opt.c.find('.tp-rbslider-slidesli');
	...
	if (_.border_radius!==undefined && parseInt(_.border_radius,0) >0) {
		punchgs.TweenLite.set(opt.c.find('.tp-rbslider-slidesli'),{borderRadius:_.border_radius});
	}		
}
...
var getActiveCarouselOffset = function(opt) {
	...
	if (_.slide_globaloffset=="none") 
		...
		_.slide_offset = 0;
		var ci = opt.c.find('.processing-rbslide').index(),
		...
		ci = ci>=0 ? ci : opt.c.find('.active-rbslide').index(); 
		ci = ci>=0 ? ci : 0;		
