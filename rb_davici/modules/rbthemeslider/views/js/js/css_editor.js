...
var UniteCssEditorRb = new function(){
	...
	var init50Editor = function(){
		 ...
		 **/
		jQuery('.rb-advanced-css-idle, .rb-advanced-css-hover').click(function(){
			//check if layer is selected or not
			var sel_layer = UniteLayersRb.get_current_selected_layer();
			
			if(sel_layer === -1) return false;
			
			
			if(!t.checkIfHandleExists(jQuery('#layer_caption').val())){
				alert(rb_lang.please_select_first_an_existing_style);
				return false;
			}
			
			cur_editing = (jQuery(this).hasClass('rb-advanced-css-idle')) ? 'idle' : 'hover';
		 ...
		 **/
		jQuery('.rb-advanced-css-idle-layer, .rb-advanced-css-hover-layer').click(function(){
			//check if layer is selected or not
			var sel_layer = UniteLayersRb.get_current_selected_layer();
			
			if(sel_layer === -1) return false;
			
			cur_editing = (jQuery(this).hasClass('rb-advanced-css-idle-layer')) ? 'idle' : 'hover';
	...
	t.setPreviewTextClass = function(){
		if(typeof(curFullClass['handle']) !== 'undefined'){
			...
			for(var key in use_class){
				jQuery('#rb-example-style-layer').addClass(use_class[key]);
			}
		}
		jQuery('#rb-example-style-layer').parent().show();
	}
	...
	t.clearPreviewText = function(){
		jQuery('#rb-example-style-layer').attr('style', '');
		jQuery('#rb-example-style-layer').attr('class', '');
	}
	...
	t.setTemplateCssLayer = function(){
		...
		var layer = UniteLayersRb.getCurrentLayer();
	...
	t.saveStylesInDb = function(handle, new_style, dialog_obj){
		...
		layer = UniteLayersRb.getCurrentLayer();
		...
		UniteAdminRb.ajaxRequest(donow,data,function(response){
			
			UniteLayersRb.setCaptionClasses(response.arrCaptions);
	...
	t.saveTemplateStylesInDb = function(){
		...
		UniteAdminRb.ajaxRequest("update_captions_advanced_css",data,function(response){
	...
	var updateLayerCSS = function(){
		...
		tpLayerTimelinesRb.rebuildLayerIdle(jQuery('.slide_layer.layer_selected'));
	...
	t.renameStylesInDb = function(old_name, new_name){
		...
		UniteAdminRb.ajaxRequest("rename_captions_css",data,function(response){
			...
			var layers = UniteLayersRb.getSimpleLayers();
			
			for(var key in layers){
				if(layers[key].style == old_name){
					UniteLayersRb.updateLayer(key, objUpdate);
				}
			}
	...
	t.deleteStylesInDb = function(handle, id){
		...
		UniteAdminRb.ajaxRequest("delete_captions_css",handle,function(response){
