...
var UniteAdminRb = new function(){
	...
	********************************/
	...
	t.ajaxRequest = function(action,data,successFunction,hideOverlay){
		...
		if(hideOverlay===undefined && typeof showWaitAMinute != 'undefined')
		...
		jQuery.ajax({
			...
			success:function(response){
				...
				if(hideOverlay===undefined && !response.is_redirect && typeof showWaitAMinute != 'undefined')
				...
				if(response.is_redirect){
					location.href=response.redirect_url; // redirect after action. temporarily disabled, but need to active.
                }
	...
	var initVideoDialog = function(){
		...
		jQuery("#video_radio_vimeo").click(function(){
			...
			jQuery("#rb-html5-options").hide();
			jQuery("#rb-youtube-options").hide();
			jQuery("#video_hidden_controls").hide();
			jQuery("#rb-video-loop-wrap").hide();
		...
		jQuery("#video_radio_youtube").click(function(){
			...
			jQuery("#video_block_html5").hide();
			jQuery("#rb-html5-options").hide();
			jQuery("#rb-youtube-options").show();
			jQuery("#video_hidden_controls").hide();
			jQuery("#rb-video-loop-wrap").hide();
		...
		jQuery("#video_radio_html5").click(function(){
			...
			jQuery("#rb-youtube-options").hide();
			jQuery("#rb-html5-options").show();
			jQuery("#video_content").hide();
			jQuery("#video_hidden_controls").show();
			jQuery("#rb-video-loop-wrap").show();
		...
		jQuery("#button_youtube_search").click(function(){
			...
			img.onload = function() {
				var img = jQuery(this);
				UniteAdminRb.onYoutubeCallback(img);
			};
			...
			//handle not found:
			setTimeout("UniteAdminRb.videoDialogOnError()",2000);
		});
		...
		jQuery("#button_vimeo_search").click(function(){
			...
			var urlAPI = 'http://www.vimeo.com/api/v2/video/' + vimeoID + '.json?callback=UniteAdminRb.onVimeoCallback';
	...
	var initGeneralSettings = function(){
		...
		jQuery("#button_save_general_settings").click(function(){
			var data = UniteSettingsRb.getSettingsObject("form_general_settings");
			jQuery("#loader_general_settings").show();
			UniteAdminRb.ajaxRequest("update_general_settings",data,function(response){
				jQuery("#loader_general_settings").hide();
	...
	var initUpdateRoutine = function(){
		...
		jQuery('#rs-validation-activate').click(function(){
			...
			UniteAdminRb.setAjaxLoaderID("rs_purchase_validation");
			UniteAdminRb.setAjaxHideButtonID("rs-validation-activate");
			...
			UniteAdminRb.ajaxRequest("activate_purchase_code",data);
		});

		jQuery('#rs-validation-deactivate').click(function(){

			UniteAdminRb.setAjaxLoaderID("rs_purchase_validation");
			UniteAdminRb.setAjaxHideButtonID("rs-validation-deactivate");

			UniteAdminRb.ajaxRequest("deactivate_purchase_code",'');
	...
	jQuery(document).ready(function(){
		...
		jQuery("#button_update_text").click(function(){
			UniteAdminRb.ajaxRequest("update_text","",function(response){
	...
	t.initGoogleFonts = function(){
		...
		jQuery('#eg-font-add').click(function(){
			jQuery('#font-dialog-wrap').dialog({
				...
				buttons: [ { text: 'Add Font', click: function() {
					...
					UniteAdminRb.ajaxRequest("add_google_fonts", data, function(response){}); //'#eg-font-add',
		...
		jQuery('body').on('click', '.eg-font-edit', function(){
			if(confirm('Really change font settings?')){
				...
				UniteAdminRb.ajaxRequest("edit_google_fonts", data, function(response){}); //'#eg-font-add, .eg-font-edit, .eg-font-delete',
			}
		...
		jQuery('body').on('click', '.eg-font-delete', function(){
			if(confirm('Really delete font?')){
				...
				UniteAdminRb.ajaxRequest("remove_google_fonts", data, function(response){ //'#eg-font-add, .eg-font-edit, .eg-font-delete',
	...
	t.inithooksetting = function(){
		...
		jQuery('#eg-hook-add').click(function(){
			jQuery('#hook-dialog-wrap').dialog({
				...
				buttons: [ { text: 'Add Hook', click: function() {
					...
					UniteAdminRb.ajaxRequest("add_new_hook", data, function(response){});
				}}],
			});
		});
		...
		jQuery('body').on('click', '.eg-hook-delete', function(){
			if(confirm('Really delete Hook?')){
				...
				UniteAdminRb.ajaxRequest("removes_hooks", data, function(response){
...
function trace(data,clear){
	UniteAdminRb.trace(data,clear);
}

function debug(data){
	UniteAdminRb.debug(data);
}

$(document).ready(function(){
	UniteAdminRb.d_none_rb_by_id("jquery_noconflict_row");
	UniteAdminRb.d_none_rb_by_id("js_to_body_row");
});
