...
var tpLayerTimelinesRb = new function(){
	var t = this,
		u = new Object(),
		mainMaxTimeLeft,
	...
	t.init = function() {
		...
		jQuery('body').on('click','.remove-trans-from-list',function() {
			...
			if (found && jQuery('.remove-trans-from-list').length>1) {
				...
				alert(rb_lang.cant_remove_last_transition);
			}
		...
		jQuery('.input-deepselects').each(function() {
			...
			dl.append(txt);
			if (rb=="on") {
				RbSliderSettings.onoffStatus(jQuery('input[name="'+id+'_reverse"]'));
			}
		})
	...
	******************************************/
	...
	t.rebuildLayerIdle = function(caption,timer,isDemo) {
		...
		if (e_img.length>0 && !jQuery(e_img).hasClass("loaded")) {
			...
			img.onerror = function() {				
				e_img[0].src = objLayer.image_url = g_rbslider_url+"/admin/assets/images/tp-brokenimage.png";
				u.updateHtmlLayerPosition(false,caption,objLayer,u.getVal(objLayer, 'top'),u.getVal(objLayer, 'left'),u.getVal(objLayer, 'align_hor'),u.getVal(objLayer, 'align_vert'));
			};
	...
	t.rebuildLayerIdleProgress = function(caption) {
		...
		// BACKGROUND OPACITY
		if (Number(bgtrans)<1) {
			var rgb = UniteAdminRb.convertHexToRGB(bgcolor);
			bgcolor="rgba("+rgb[0]+","+rgb[1]+","+rgb[2]+","+bgtrans+")";
		}

		// BORDER OPACITY
		if (Number(bordertrans)<1) {
			var rgb = UniteAdminRb.convertHexToRGB(bordercolor);
			bordercolor="rgba("+rgb[0]+","+rgb[1]+","+rgb[2]+","+bordertrans+")";
		}

		// FONT OPACITY
		if (Number(fonttrans)<1) {
			var rgb = UniteAdminRb.convertHexToRGB(fontcolor);
	...
	********************************/
	t.addToSortbox = function(serial,objLayer){
		...
		switch (objLayer.type) {
			case "text":
				quicksb += '<span class="layer-short-tool rbdarkgray layer-title-with-icon"><i class="rs-icon-layerfont_n"></i>';
				btlist += '<span id="button_edit_layer_'+serial+'" class="button_edit_layer layer-short-tool rbblue"><i class="eg-icon-pencil"></i></span>';
				btlist += '<span id="button_reset_size_'+serial+'" class="button_reset_size layer-short-tool rbblue"><i class="eg-icon-resize-normal"></i></span>';
			break;	
			case "shape":
				quicksb += '<span class="layer-short-tool rbdarkgray layer-title-with-icon"><i class="rs-icon-layershape_n"></i>';
				btlist += '<span id="button_edit_shape_'+serial+'" class="button_edit_shape layer-short-tool rbblue"><i class="eg-icon-pencil"></i></span>';
				btlist += '<span  class="layer-short-tool rbdarkgray"></span>';
			break;
			case "button":
				quicksb += '<span class="layer-short-tool rbdarkgray layer-title-with-icon"><i class="rs-icon-layerbutton_n"></i>';
				btlist += '<span id="button_edit_layer_'+serial+'" class="button_edit_layer layer-short-tool rbblue"><i class="eg-icon-pencil"></i></span>';
				btlist += '<span id="button_reset_size_'+serial+'" class="button_reset_size layer-short-tool rbblue"><i class="eg-icon-resize-normal"></i></span>';
			break;
			case "image":
				quicksb += '<span class="layer-short-tool rbdarkgray layer-title-with-icon"><i class="rs-icon-layerimage_n"></i>';
				btlist += '<span  id="button_change_image_source_'+serial+'" class="button_change_image_source layer-short-tool rbblue"><i class="eg-icon-pencil"></i></span>';
				btlist += '<span id="button_reset_size_'+serial+'" class="button_reset_size layer-short-tool rbblue"><i class="eg-icon-resize-normal"></i></span>';
			break;
			case "video":
				quicksb += '<span class="layer-short-tool rbdarkgray layer-title-with-icon"><i class="rs-icon-layervideo_n"></i>';
				btlist += '<span  id="button_change_video_settings_'+serial+'" class="button_change_video_settings layer-short-tool rbblue"><i class="eg-icon-pencil"></i></span>';				
				btlist += '<span  class="layer-short-tool rbdarkgray"></span>';
			break;
		}
		...
		quicksb += btlist;		
		quicksb += '<span id="button_delete_layer_'+serial+'" class="button_delete_layer layer-short-tool rbred"><i class="rs-lighttrash"></i></span>';
		quicksb += '<span id="button_duplicate_layer_'+serial+'" class="button_duplicate_layer layer-short-tool rbyellow" data-isstatic=""><i class="rs-lightcopy"></i></span>';
		quicksb += '<span style="display:block;float:none;clear:both"></span></span>';
		quicksb += '<span class="quick-layer-view layer-short-tool rbdarkgray '+visibleclass+'"><i class="eg-icon-eye"></i></span>';
		quicksb += '<span class="quick-layer-lock layer-short-tool rbdarkgray"><i class="eg-icon-lock-open"></i></span>';
	...
	*/
	...
	t.getSortboxText = function(text){
		sorboxTextSize = 20;
		var textSortbox = UniteAdminRb.stripTags(text);

		//if no content - escape html
		if(textSortbox.length < 2)
			textSortbox = UniteAdminRb.htmlspecialchars(text);
	...
	************************************/
	...
	t.getHtmlSortItemFromSerial = function(serial){
		...
		if(htmlSortItem.length == 0){
			UniteAdminRb.showErrorMessage("Html sort field with serial: "+serial+" not found!");
			return(false);
		}
		return(htmlSortItem);
	}
