...
class AdminRbthemesliderUploadController extends ModuleAdminController
{
    ...
    public function initProcess()
    {
        ...
        if (@Rbthemeslider::getIsset($view) && !empty($view)) {
            $file = _PS_ROOT_DIR_.'/modules/rbthemeslider/views/'.$view.'.php';
    ...
    public function initHeader()
    {
        ...
        foreach ($quick_access as $index => $quick) {
            if ($quick['link'] == '../' && Shop::getContext() == Shop::CONTEXT_SHOP) {
                ...
                if (@Rbthemeslider::getIsset($admin_tab[1])) {
                    ...
                    $token = Tools::getAdminToken(
                        $admin_tab[1].(int)Tab::getIdFromClassName($admin_tab[1]).(int)$this->context->employee->id
                    );
        ...
        if (@Rbthemeslider::getIsset($this->context->smarty->tpl_vars['breadcrumbs2']) &&
            $this->context->smarty->tpl_vars['breadcrumbs2']->value['tab']['name']
        ) {
            if ($this->context->smarty->tpl_vars['breadcrumbs2']->value['action']['name']) {
                ...
                $this->context->smarty->tpl_vars['breadcrumbs2']->value['action']['name'];
            } else {
                $name = $this->context->smarty->tpl_vars['breadcrumbs2']->value['tab']['name'];
            }
        } elseif (@Rbthemeslider::getIsset($this->context->smarty->tpl_vars['breadcrumbs2']) &&
            is_string($this->context->smarty->tpl_vars['breadcrumbs2']->value)
        ) {
        ...
        $quick_access[] = array(
            ...
            'link' => $this->context->link->getAdminLink('AdminQuickAccesses').'&new_window=0&name_'.
            (int)Configuration::get('PS_LANG_DEFAULT').'='.urlencode($name).'&link='.
            urlencode($link).'&submitAddquick_access=1',
            'new_window' => 0
        );
        ...
        foreach ($tabs as $index => $tab) {
            ...
            if (!$img_exists_cache) {
                $img_exists_cache = Tools::file_exists_cache(
                    _PS_ADMIN_DIR_.str_replace('.png', '.gif', $img_cache_url)
                );
            }
            ...
            $tabs[$index]['current'] = ($tab['class_name'].'Controller' == get_class($this))
            ...
            foreach ($sub_tabs as $index2 => $sub_tab) {
                ...
                if (@Rbthemeslider::getIsset($sub_tab['module']) && !empty($sub_tab['module'])) {
                ...
                if (Tab::checkTabRights($sub_tab['id_tab']) === true &&
                    (bool)$sub_tab['active'] &&
                    $sub_tab['class_name'] != 'AdminCarrierWizard'
                ) {
                    // class_name is the name of the class controller
                    $sub_tabs[$index2]['href'] = $this->context->link->getAdminLink($sub_tab['class_name']);
                    $sub_tabs[$index2]['current'] = ($sub_tab['class_name'].'Controller' ==
                    get_class($this) || $sub_tab['class_name'] == Tools::getValue('controller'));
                } elseif ($sub_tab['class_name'] == 'AdminCarrierWizard' &&
                    $sub_tab['class_name'].'Controller' == get_class($this)
                ) {
        ...
        if (Validate::isLoadedObject($this->context->employee)) {
            ...
            $this->context->smarty->assign(array(
                ...
                'bo_color' => @Rbthemeslider::getIsset($this->context->employee->bo_color) ?
                Tools::htmlentitiesUTF8($this->context->employee->bo_color) : null,
                'show_new_orders' => Configuration::get('PS_SHOW_NEW_ORDERS') &&
                @Rbthemeslider::getIsset($accesses['AdminOrders']) &&
                $accesses['AdminOrders']['view'],
                'show_new_customers' => Configuration::get('PS_SHOW_NEW_CUSTOMERS') &&
                @Rbthemeslider::getIsset($accesses['AdminCustomers']) &&
                $accesses['AdminCustomers']['view'],
                'show_new_messages' => Configuration::get('PS_SHOW_NEW_MESSAGES') &&
                @Rbthemeslider::getIsset($accesses['AdminCustomerThreads']) &&
                $accesses['AdminCustomerThreads']['view'],
                ...
                'default_tab_link' => $this->context->link->getAdminLink(
                    Tab::getClassNameById((int)Context::getContext()->employee->default_tab)
                ),
                'collapse_menu' => @Rbthemeslider::getIsset($this->context->cookie->collapse_menu) ?
                (int)$this->context->cookie->collapse_menu : 0
            ));
        } else {
            $this->context->smarty->assign(
                'default_tab_link',
                $this->context->link->getAdminLink('AdminDashboard')
            );
        }
        ...
        $this->context->smarty->assign(array(
            ...
            'tab' => @Rbthemeslider::getIsset($tab) ? $tab : null,
            'current_parent_id' => (int)Tab::getCurrentParentId(),
            'tabs' => $tabs,
        ...
        if (is_object($module) &&
            $module->active &&
            (int)Configuration::get('PS_TC_ACTIVE') == 1 &&
